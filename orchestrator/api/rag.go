...
func (h *RAGHandler) HandleRaptorBuild(w http.ResponseWriter, r *http.Request) {
	...
	tree, err := h.engine.GetRaptor().BuildTree(r.Context(), req.Papers, req.MinClusters)
	if err != nil {
		WriteError(w, http.StatusInternalServerError, ErrRagFailed, "raptor build failed", map[string]any{
			"error": err.Error(),
		})
		return
	}
...
func (h *RAGHandler) HandleMultiAgent(w http.ResponseWriter, r *http.Request) {
	...
	ragReq := rag.AnswerRequest{
		Query:  req.Query,
		Domain: req.DomainHint,
		Limit:  15,
	}

	resp, err := h.engine.MultiAgentExecute(r.Context(), ragReq)
	if err != nil {
