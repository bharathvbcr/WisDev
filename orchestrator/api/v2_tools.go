...
func (s *wisdevV2Server) registerToolRoutes(mux *http.ServeMux, agentGateway *wisdev.AgentGateway) {
	toolSearchHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		ranked := wisdev.RankTools(query, agentGateway.Registry.List(), req.Limit)
		payload := map[string]any{
			"query": query,
			"tools": ranked,
			"count": len(ranked),
		}
		traceID := writeV2Envelope(w, "toolSearch", payload)
		s.journalEvent("tool_search", r.URL.Path, traceID, "", "", "", "", "Tool search completed.", payload, nil)
	}

	mux.HandleFunc("/v2/wisdev/tool-search", toolSearchHandler)
	mux.HandleFunc("/v2/wisdev/wisdev.Tool-search", toolSearchHandler)
	...
	mux.HandleFunc("/v2/wisdev/structured-output", func(w http.ResponseWriter, r *http.Request) {
		...
		traceID := writeV2EnvelopeStatus(w, status, "structuredOutput", payload)
		s.journalEvent("structured_output", "/v2/wisdev/structured-output", traceID, "", "", "", "", "Structured output validated.", payload, nil)
	})
}
