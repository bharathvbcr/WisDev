...
func TestFirstNChars(t *testing.T) {
	t.Run("returns_original_when_shorter", func(t *testing.T) {
		assert.Equal(t, "hello", firstNChars("hello", 10))
	})

	t.Run("returns_exact_prefix", func(t *testing.T) {
		assert.Equal(t, "hel", firstNChars("hello", 3))
	})

	t.Run("negative_or_zero_n", func(t *testing.T) {
		assert.Equal(t, "", firstNChars("hello", 0))
		assert.Panics(t, func() {
			_ = firstNChars("hello", -1)
		})
	})
}

func TestMaxInt(t *testing.T) {
	t.Run("returns_first_when_greater", func(t *testing.T) {
		assert.Equal(t, 5, maxInt(5, 3))
	})

	t.Run("returns_second_when_greater", func(t *testing.T) {
		assert.Equal(t, 7, maxInt(2, 7))
	})

	t.Run("returns_either_when_equal", func(t *testing.T) {
