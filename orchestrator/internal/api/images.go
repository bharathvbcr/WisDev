...
var imageGenerationRequestTimeout = 45 * time.Second
...
func imageGenerationContext(ctx context.Context) (context.Context, context.CancelFunc) {
	if imageGenerationRequestTimeout <= 0 {
		return ctx, func() {}
	}
	return context.WithTimeout(ctx, imageGenerationRequestTimeout)
}
...
func (h *ImageHandler) HandleGenerate(w http.ResponseWriter, r *http.Request) {
	...
	startTime := time.Now()
	genCtx, genCancel := imageGenerationContext(r.Context())
	defer genCancel()

	images, err := h.vertex.GenerateImages(genCtx, "", req.Prompt, req.Count, req.AspectRatio)
	if err != nil {
