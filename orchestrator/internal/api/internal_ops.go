...
func (h *InternalOpsHandler) HandleAccountDelete(w http.ResponseWriter, r *http.Request) {
	...
	if !alreadyDeleted {
		if err := h.deleteUserFromFirebase(r.Context(), req.UserID); err != nil {
			slog.Error("failed to delete user from firebase", "error", err, "userId", req.UserID)
...
func (h *InternalOpsHandler) deleteUserFromFirebase(ctx context.Context, userID string) error {
	service, err := identitytoolkit.NewService(ctx, option.WithScopes("https://www.googleapis.com/auth/firebase", "https://www.googleapis.com/auth/cloud-platform"))
	if err != nil {
		return err
	}

	// Identity Toolkit v3 deleteAccount request.
	_, err = service.Relyingparty.DeleteAccount(&identitytoolkit.IdentitytoolkitRelyingpartyDeleteAccountRequest{
		LocalId: userID,
	}).Context(ctx).Do()
	return err
}
