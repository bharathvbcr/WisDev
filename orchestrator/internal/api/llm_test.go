...
func testHeavyModel() string {
	return llm.ResolveHeavyModel()
}

func testStandardModel() string {
	return llm.ResolveStandardModel()
}

func testLightModel() string {
...
type scriptedLLMClient struct {
	...
	respQueue               []*llmv1.GenerateResponse
	callOrder               []string
...
func TestLLMHandlerResolveModel(t *testing.T) {
	...
	if got := handler.resolveModel("heavy"); got != testHeavyModel() {
		t.Fatalf("resolveModel(heavy) = %q, want %q", got, testHeavyModel())
	}
	if got := handler.resolveModel("light"); got != testLightModel() {
		t.Fatalf("resolveModel(light) = %q, want %q", got, testLightModel())
	}
	if got := handler.resolveModel("standard"); got != testStandardModel() {
		t.Fatalf("resolveModel(standard) = %q, want %q", got, testStandardModel())
	}
	if got := handler.resolveModel("unknown"); got != testStandardModel() {
		t.Fatalf("resolveModel(unknown) = %q, want %q", got, testStandardModel())
	}
}
...
func TestLLMHandlerHandleGenerate(t *testing.T) {
	...
	t.Run("defaults to standard when no tier provided", func(t *testing.T) {
		...
		if got := client.callOrder[0]; got != testStandardModel() {
			t.Fatalf("expected first attempt model %q, got %q", testStandardModel(), got)
		}
	})
	...
	t.Run("defaults to light for light task type", func(t *testing.T) {
		...
		if got := client.callOrder[0]; got != testLightModel() {
			t.Fatalf("expected first attempt model %q, got %q", testLightModel(), got)
		}
	})
	...
	t.Run("routes json_object requests through StructuredOutput and preserves schema intent", func(t *testing.T) {
		...
		if got := client.structuredCallOrder[0]; got != testStandardModel() {
			t.Fatalf("expected first structured attempt model %q, got %q", testStandardModel(), got)
		}
	...
	t.Run("structured requests fall back when the first tier returns invalid JSON", func(t *testing.T) {
		...
		if got := client.structuredCallOrder[0]; got != testLightModel() {
			t.Fatalf("expected first structured attempt model %q, got %q", testLightModel(), got)
		}
		if got := client.structuredCallOrder[1]; got != testStandardModel() {
			t.Fatalf("expected second structured attempt model %q, got %q", testStandardModel(), got)
		}
		body := strings.TrimSpace(rec.Body.String())
	...
	t.Run("structured requests fall back when the sidecar wraps provider failure in invalid argument", func(t *testing.T) {
		client := &scriptedLLMClient{
			structuredErrorsByModel: map[string]error{
				testLightModel(): status.Error(codes.InvalidArgument, `{"error":{"code":"STRUCTURED_FAILED","message":"structured output returned invalid JSON"}}`),
			},
			structuredRespByModel: map[string]*llmv1.StructuredResponse{
				testStandardModel(): {
					JsonResult:  `{"ok":true}`,
	...
	t.Run("falls back to next tier on failures", func(t *testing.T) {
		client := &scriptedLLMClient{
			errorsByModel: map[string]error{
				testHeavyModel():    assertError("heavy failure"),
				testStandardModel(): assertError("standard failure"),
			},
			respByModel: map[string]*llmv1.GenerateResponse{
				testLightModel(): {
		...
		if got := client.callOrder[2]; got != testLightModel() {
			t.Fatalf("expected third attempt model %q, got %q", testLightModel(), got)
		}
		body := strings.TrimSpace(rec.Body.String())
	...
	t.Run("falls back when first attempt returns blank output", func(t *testing.T) {
		...
		if got := client.callOrder[1]; got != testStandardModel() {
			t.Fatalf("expected second attempt model %q, got %q", testStandardModel(), got)
		}
		body := strings.TrimSpace(rec.Body.String())
	...
	t.Run("returns bad gateway after all failures", func(t *testing.T) {
		client := &scriptedLLMClient{
			errorsByModel: map[string]error{
				testStandardModel(): assertError("standard failure"),
				testHeavyModel():    assertError("heavy failure"),
				testLightModel():    assertError("light failure"),
			},
		}
		handler := NewLLMHandler(client)
...
func TestLLMHandlerHandleGenerate_ErrorKinds(t *testing.T) {
	t.Run("grpc invalid argument errors stop immediately as permanent", func(t *testing.T) {
		client := &scriptedLLMClient{
			errorsByModel: map[string]error{
				testStandardModel(): status.Error(codes.InvalidArgument, `{"error":{"code":"INVALID_PROMPT","message":"prompt is required"}}`),
			},
		}
		handler := NewLLMHandler(client)
	...
	t.Run("permanent errors stop immediately", func(t *testing.T) {
		client := &scriptedLLMClient{
			errorsByModel: map[string]error{
				testStandardModel(): assertError("permission denied"),
			},
		}
		handler := NewLLMHandler(client)
		req := httptest.NewRequest(http.MethodPost, "/llm/generate", strings.NewReader(`{"prompt":"analyze topic","tier":"standard"}`))
	...
	t.Run("rate limit errors map to 429", func(t *testing.T) {
		client := &scriptedLLMClient{
			errorsByModel: map[string]error{
				testStandardModel(): assertError("rate limit 429"),
				testHeavyModel():    assertError("rate limit 429"),
				testLightModel():    assertError("rate limit 429"),
			},
		}
		handler := NewLLMHandler(client)
		req := httptest.NewRequest(http.MethodPost, "/llm/generate", strings.NewReader(`{"prompt":"analyze topic","tier":"standard"}`))
	...
	t.Run("timeout errors map to 504", func(t *testing.T) {
		client := &scriptedLLMClient{
			errorsByModel: map[string]error{
				testStandardModel(): assertError("context deadline exceeded"),
				testHeavyModel():    assertError("context deadline exceeded"),
				testLightModel():    assertError("context deadline exceeded"),
			},
		}
		handler := NewLLMHandler(client)
		req := httptest.NewRequest(http.MethodPost, "/llm/generate", strings.NewReader(`{"prompt":"analyze topic","tier":"standard"}`))
