...
func TestNewRouter_WiresWisDevExecuteRoute(t *testing.T) {
	t.Setenv("INTERNAL_SERVICE_KEY", "test-internal-key")

	router := NewRouter(ServerConfig{Version: "test"})
	req := httptest.NewRequest(http.MethodPost, "/wisdev/execute", strings.NewReader(`{"action":"noop","payload":{}}`))
...
func TestNewRouter_RemovesRootSearchAndAiCompatibilityAliases(t *testing.T) {
	t.Setenv("INTERNAL_SERVICE_KEY", "test-internal-key")
	...
	requests := []struct {
		...
		{method: http.MethodGet, path: "/search?q=sleep"},
		{method: http.MethodPost, path: "/ai", body: `{"prompt":"test"}`},
		{method: http.MethodPost, path: "/api/v1/ai", body: `{"prompt":"test"}`},
		{method: http.MethodPost, path: "/api/v1/ai/generate", body: `{"prompt":"test"}`},
		{method: http.MethodPost, path: "/api/v1/export/markdown", body: `{"paper":{"title":"T"}}`},
		{method: http.MethodPost, path: "/api/v1/export/html", body: `{"paper":{"title":"T"}}`},
		{method: http.MethodPost, path: "/api/v1/export/latex", body: `{"paper":{"title":"T"}}`},
		{method: http.MethodPost, path: "/api/v1/images/generate", body: `{"prompt":"diagram"}`},
		{method: http.MethodGet, path: "/api/v1/papers/network"},
		{method: http.MethodGet, path: "/external/proxy?readiness=1"},
		{method: http.MethodPost, path: "/api/rag/extraction/paper", body: `{"paper":{"title":"T"}}`},
		{method: http.MethodPost, path: "/rag?action=embed", body: `{"text":"policy"}`},
		{method: http.MethodPost, path: "/rag?action=embed-batch", body: `{"texts":["alpha","beta"]}`},
		{method: http.MethodPost, path: "/runtime/telemetry/delete", body: `{"sessionId":"s1"}`},
	}
