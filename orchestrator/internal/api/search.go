...
func (h *SearchHandler) HandleParallelSearch(w http.ResponseWriter, r *http.Request) {
	...
	logSearchRouteLifecycle(r, "search_parallel", "request_received", firstNonEmptyTrimmed(r.URL.Query().Get("q"), r.URL.Query().Get("query")), traceID,
		"result", "received",
	)
	if r.Method != http.MethodGet && r.Method != http.MethodPost {
		logSearchRouteLifecycle(r, "search_parallel", "method_rejected", "", traceID,
			"result", "failure",
	...
	if err != nil {
		logSearchRouteLifecycle(r, "search_parallel", "validation_failed", query, traceID,
			"result", "failure",
	...
	traceID = firstNonEmptyTrimmed(opts.TraceID, traceID)
	logSearchRouteLifecycle(r, "search_parallel", "request_validated", query, traceID,
		"limit", opts.Limit,
		"domain", opts.Domain,
	...
	logSearchRouteLifecycle(r, "search_parallel", "dispatch_start", query, traceID,
		"strategy", "wisdev_parallel_search",
	)
	...
	if err != nil {
		logSearchRouteLifecycle(r, "search_parallel", "dispatch_failed", query, traceID,
			"result", "failure",
	...
	logSearchRouteLifecycle(r, "search_parallel", "response_ready", response.QueryUsed, response.TraceID,
		"result", "success",
		"result_count", len(response.Papers),
		"provider_count", len(response.Providers),
		"warning_count", len(response.Warnings),
		"latency_ms", response.LatencyMs,
		"cache_hit", response.CacheHit,
		"fallback_triggered", response.Metadata.FallbackTriggered,
	)
...
func (h *SearchHandler) HandleBatchSearch(w http.ResponseWriter, r *http.Request) {
	traceID := resolveRequestTraceID(r)
	logSearchRouteLifecycle(r, "search_batch", "request_received", "", traceID,
		"result", "received",
	)
	if r.Method != http.MethodPost {
		logSearchRouteLifecycle(r, "search_batch", "method_rejected", "", traceID,
			"result", "failure",
			"error_code", "METHOD_NOT_ALLOWED",
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		logSearchRouteLifecycle(r, "search_batch", "body_decode_failed", "", traceID,
			"result", "failure",
	...
	if len(req.Queries) == 0 {
		logSearchRouteLifecycle(r, "search_batch", "validation_failed", "", traceID,
			"result", "failure",
			"error_code", "EMPTY_QUERIES",
		)
	...
	logSearchRouteLifecycle(r, "search_batch", "request_validated", "", traceID,
		"query_count", len(req.Queries),
		"limit", limit,
	...
	for _, query := range req.Queries {
		...
		if query == "" {
			logSearchRouteLifecycle(r, "search_batch", "query_skipped_empty", query, traceID,
				"result", "skipped",
			)
			continue
		}
		...
		go func() {
			...
			if err := sem.Acquire(r.Context(), 1); err != nil {
				logSearchRouteLifecycle(r, "search_batch", "query_dispatch_cancelled", query, traceID,
					"result", "failure",
					"error_code", "CONTEXT_CANCELLED",
					"error", err.Error(),
				)
			...
			logSearchRouteLifecycle(r, "search_batch", "query_dispatch_start", query, traceID,
				"limit", limit,
			)
			...
			logSearchRouteLifecycle(r, "search_batch", "query_dispatch_complete", query, traceID,
				"result", "success",
	...
	logSearchRouteLifecycle(r, "search_batch", "response_ready", "", traceID,
		"result", "success",
		"result_query_count", len(resultsMap),
	)
...
func (h *SearchHandler) HandleHybridSearch(w http.ResponseWriter, r *http.Request) {
	traceID := resolveRequestTraceID(r)
	logSearchRouteLifecycle(r, "search_hybrid", "request_received", firstNonEmptyTrimmed(r.URL.Query().Get("q"), r.URL.Query().Get("query")), traceID,
		"result", "received",
	)
	if r.Method != http.MethodGet && r.Method != http.MethodPost {
		logSearchRouteLifecycle(r, "search_hybrid", "method_rejected", "", traceID,
			"result", "failure",
			"error_code", "METHOD_NOT_ALLOWED",
			"allowed_methods", "GET,POST",
		)
	...
	if err != nil {
		logSearchRouteLifecycle(r, "search_hybrid", "validation_failed", "", traceID,
			"result", "failure",
			"error_code", "INVALID_REQUEST",
			"error", err.Error(),
		)
	...
	traceID = firstNonEmptyTrimmed(req.TraceID, traceID)
	...
	logSearchRouteLifecycle(r, "search_hybrid", "request_validated", req.Query, traceID,
		"backend", backend,
		"limit", req.Limit,
		"domain", req.Domain,
		"source_count", len(req.Sources),
		"latency_budget_ms", req.LatencyBudgetMs,
	)
	...
	err = withConcurrencyGuard("gateway_search", wisdev.EnvInt("WISDEV_GATEWAY_SEARCH_CONCURRENCY", 30), func() error {
		...
		case "opensearch_hybrid", "opensearch-hybrid", "opensearch":
			logSearchRouteLifecycle(r, "search_hybrid", "opensearch_dispatch_start", req.Query, traceID,
				"backend", backend,
			)
			if h.openSearch == nil {
			...
			if result.FallbackTriggered {
				logSearchRouteLifecycle(r, "search_hybrid", "opensearch_fallback_start", req.Query, traceID,
					"backend", backend,
		...
		default:
			logSearchRouteLifecycle(r, "search_hybrid", "parallel_dispatch_start", req.Query, traceID,
				"backend", backend,
			)
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "concurrency limit reached") {
			...
			stage = "concurrency_rejected"
		}
		logSearchRouteLifecycle(r, "search_hybrid", stage, req.Query, traceID,
			"result", "failure",
			"status", status,
			"error_code", code,
			"error", err.Error(),
	...
	w.Header().Set("X-Trace-Id", response.TraceID)
	logSearchRouteLifecycle(r, "search_hybrid", "response_ready", response.QueryUsed, response.TraceID,
		"result", "success",
...
func (h *SearchHandler) HandleToolSearch(w http.ResponseWriter, r *http.Request) {
	traceID := resolveRequestTraceID(r)
	logSearchRouteLifecycle(r, "search_tool", "request_received", "", traceID,
		"result", "received",
	)
	if r.Method != http.MethodPost {
		logSearchRouteLifecycle(r, "search_tool", "method_rejected", "", traceID,
			"result", "failure",
			"error_code", "METHOD_NOT_ALLOWED",
			"allowed_method", http.MethodPost,
		)
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		logSearchRouteLifecycle(r, "search_tool", "body_decode_failed", "", traceID,
			"result", "failure",
			"error_code", "INVALID_BODY",
			"error", err.Error(),
		)
	...
	if strings.TrimSpace(req.Tool) == "" {
		logSearchRouteLifecycle(r, "search_tool", "validation_failed", "", traceID,
			"result", "failure",
	...
	query := wisdev.AsOptionalString(req.Params["q"])
	logSearchRouteLifecycle(r, "search_tool", "dispatch_start", query, traceID,
		"tool", strings.TrimSpace(req.Tool),
	)
	result, err := search.HandleToolSearch(r.Context(), h.resolveRegistry(), req.Tool, req.Params)
	if err != nil {
		logSearchRouteLifecycle(r, "search_tool", "dispatch_failed", query, traceID,
			"result", "failure",
			"error_code", "TOOL_SEARCH_FAILED",
			"tool", strings.TrimSpace(req.Tool),
			"error", err.Error(),
		)
	...
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("X-Trace-Id", traceID)
	logSearchRouteLifecycle(r, "search_tool", "response_ready", query, traceID,
		"result", "success",
		"tool", strings.TrimSpace(req.Tool),
	)
...
func (h *SearchHandler) HandleRelatedArticles(w http.ResponseWriter, r *http.Request) {
	traceID := resolveRequestTraceID(r)
	logSearchRouteLifecycle(r, "search_related_articles", "request_received", "", traceID,
		"result", "received",
	)
	if r.Method != http.MethodPost {
		logSearchRouteLifecycle(r, "search_related_articles", "method_rejected", "", traceID,
			"result", "failure",
			"error_code", "METHOD_NOT_ALLOWED",
			"allowed_method", http.MethodPost,
		)
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		logSearchRouteLifecycle(r, "search_related_articles", "body_decode_failed", "", traceID,
			"result", "failure",
			"error_code", string(ErrBadRequest),
			"error", err.Error(),
		)
	...
	if query == "" {
		logSearchRouteLifecycle(r, "search_related_articles", "validation_failed", query, traceID,
			"result", "failure",
			"error_code", string(ErrInvalidParameters),
		)
	...
	logSearchRouteLifecycle(r, "search_related_articles", "dispatch_start", searchQuery, traceID,
		"source_title_present", sourceTitle != "",
		"existing_link_count", len(req.ExistingLinks),
	)
	hits, err := wisdev.FastParallelSearch(r.Context(), h.redis, searchQuery, 8)
	if err != nil {
		logSearchRouteLifecycle(r, "search_related_articles", "dispatch_failed", searchQuery, traceID,
			"result", "failure",
			"error_code", string(ErrWisdevFailed),
			"error", err.Error(),
		)
	...
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("X-Trace-Id", traceID)
	logSearchRouteLifecycle(r, "search_related_articles", "response_ready", searchQuery, traceID,
		"result", "success",
...
func (h *SearchHandler) HandleQueryField(w http.ResponseWriter, r *http.Request) {
	traceID := resolveRequestTraceID(r)
	logSearchRouteLifecycle(r, "search_query_field", "request_received", "", traceID,
		"result", "received",
	)
	if r.Method != http.MethodPost {
		logSearchRouteLifecycle(r, "search_query_field", "method_rejected", "", traceID,
			"result", "failure",
			"error_code", string(ErrBadRequest),
			"allowed_method", http.MethodPost,
		)
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		logSearchRouteLifecycle(r, "search_query_field", "body_decode_failed", "", traceID,
			"result", "failure",
			"error_code", string(ErrBadRequest),
			"error", err.Error(),
		)
	...
	query := strings.TrimSpace(req.Query)
	if query == "" {
		logSearchRouteLifecycle(r, "search_query_field", "validation_failed", query, traceID,
			"result", "failure",
			"error_code", string(ErrInvalidParameters),
		)
		WriteError(w, http.StatusBadRequest, ErrInvalidParameters, "query is required", nil)
	...
	logSearchRouteLifecycle(r, "search_query_field", "classification_start", query, traceID)
	fieldID, confidence := ClassifyQueryField(query)
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("X-Trace-Id", traceID)
	logSearchRouteLifecycle(r, "search_query_field", "response_ready", query, traceID,
		"result", "success",
		"field_id", fieldID,
...
func (h *SearchHandler) HandleQueryIntroduction(w http.ResponseWriter, r *http.Request) {
	traceID := resolveRequestTraceID(r)
	logSearchRouteLifecycle(r, "search_query_introduction", "request_received", "", traceID,
		"result", "received",
	)
	if r.Method != http.MethodPost {
		logSearchRouteLifecycle(r, "search_query_introduction", "method_rejected", "", traceID,
			"result", "failure",
			"error_code", string(ErrBadRequest),
			"allowed_method", http.MethodPost,
		)
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		logSearchRouteLifecycle(r, "search_query_introduction", "body_decode_failed", "", traceID,
			"result", "failure",
			"error_code", string(ErrBadRequest),
			"error", err.Error(),
		)
	...
	query := strings.TrimSpace(req.Query)
	if query == "" {
		logSearchRouteLifecycle(r, "search_query_introduction", "validation_failed", query, traceID,
			"result", "failure",
			"error_code", string(ErrInvalidParameters),
		)
		WriteError(w, http.StatusBadRequest, ErrInvalidParameters, "query is required", nil)
	...
	logSearchRouteLifecycle(r, "search_query_introduction", "summary_build_start", query, traceID,
		"paper_count", len(req.Papers),
		"provider_count", len(req.ProvidersUsed),
	)
	...
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("X-Trace-Id", traceID)
	logSearchRouteLifecycle(r, "search_query_introduction", "response_ready", query, traceID,
		"result", "success",
		"paper_count", len(req.Papers),
		"provider_count", len(req.ProvidersUsed),
	)
...
func (h *SearchHandler) HandleBatchSummaries(w http.ResponseWriter, r *http.Request) {
	traceID := resolveRequestTraceID(r)
	logSearchRouteLifecycle(r, "search_batch_summaries", "request_received", "", traceID,
		"result", "received",
	)
	if r.Method != http.MethodPost {
		logSearchRouteLifecycle(r, "search_batch_summaries", "method_rejected", "", traceID,
			"result", "failure",
			"error_code", string(ErrBadRequest),
			"allowed_method", http.MethodPost,
		)
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		logSearchRouteLifecycle(r, "search_batch_summaries", "body_decode_failed", "", traceID,
			"result", "failure",
			"error_code", string(ErrBadRequest),
			"error", err.Error(),
		)
	...
	if len(req.Papers) == 0 {
		logSearchRouteLifecycle(r, "search_batch_summaries", "validation_failed", "", traceID,
			"result", "failure",
			"error_code", string(ErrInvalidParameters),
		)
	...
	logSearchRouteLifecycle(r, "search_batch_summaries", "summary_build_start", "", traceID,
		"paper_count", len(req.Papers),
	...
	w.Header().Set("Content-Type", "application/json")
	w.Header().Set("X-Trace-Id", traceID)
	logSearchRouteLifecycle(r, "search_batch_summaries", "response_ready", "", traceID,
		"result", "success",
...
func resolveSearchOptions(r *http.Request) (string, search.SearchOpts, error) {
	...
	case http.MethodPost:
		...
		return query, search.SearchOpts{
			...
			Stage2Rerank:    stage2,
			TraceID:         firstNonEmptyTrimmed(req.TraceID, resolveRequestTraceID(r)),
		}, nil
	default:
...
func resolveHybridSearchRequest(r *http.Request) (hybridSearchRequest, error) {
	...
	case http.MethodPost:
		...
		req.TraceID = firstNonEmptyTrimmed(req.TraceID, resolveRequestTraceID(r))
		if req.ExpandQuery == nil {
