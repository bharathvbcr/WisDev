...
func allowAutonomousCritique(msc *mockLLMServiceClient) {
	...
	})).Return(&llmv1.StructuredResponse{JsonResult: `{"sufficient": true, "reasoning": "covered", "nextQuery": ""}`}, nil).Maybe()
	msc.On("StructuredOutput", mock.Anything, mock.MatchedBy(func(req *llmv1.StructuredRequest) bool {
		return req != nil && strings.Contains(req.Prompt, "Generate 3 specific search queries to find evidence for this hypothesis")
	})).Return(&llmv1.StructuredResponse{JsonResult: `["sleep and memory replication evidence","sleep and memory counter evidence","sleep and memory mechanistic evidence"]`}, nil).Maybe()
	msc.On("StructuredOutput", mock.Anything, mock.MatchedBy(func(req *llmv1.StructuredRequest) bool {
		return req != nil && strings.Contains(req.Prompt, "You are evaluating a research hypothesis against collected evidence")
	})).Return(&llmv1.StructuredResponse{JsonResult: `{"score":0.82,"verdict":"supported","reasoning":"covered by test evidence","branchingDecision":"keep","missingEvidence":[],"suggestedQueries":[]}`}, nil).Maybe()
	msc.On("StructuredOutput", mock.Anything, mock.MatchedBy(func(req *llmv1.StructuredRequest) bool {
		return strings.Contains(req.Prompt, "Critique the following research draft")
	})).Return(&llmv1.StructuredResponse{JsonResult: `{"needsRevision": false, "reasoning": "grounded", "confidence": 0.82}`}, nil).Maybe()
	msc.On("StructuredOutput", mock.Anything, mock.MatchedBy(isAutonomousCommitteePrompt)).Return(&llmv1.StructuredResponse{JsonResult: `{"verdict":"approve","reason":"covered by test evidence"}`}, nil).Maybe()
...
func TestAutonomousResearchRouteLogsStageHandoffs(t *testing.T) {
	originalRunUnifiedResearchLoop := runUnifiedResearchLoop
	t.Cleanup(func() {
		runUnifiedResearchLoop = originalRunUnifiedResearchLoop
	})
	...
	for _, stage := range []string{
		...
		if !strings.Contains(logOutput, "stage="+stage) {
			t.Fatalf("expected log stage %q, got logs:\n%s", stage, logOutput)
		}
	}
	if !strings.Contains(logOutput, "trace_id=trace-stage-log") {
		t.Fatalf("expected trace id in stage logs, got logs:\n%s", logOutput)
	}
}
...
func TestAutonomousResearchRouteMarksLoopFailureAsDegradedFallback(t *testing.T) {
	...
	registry.Register(&mockSearchProvider{
		...
		SearchFunc: func(_ context.Context, query string, opts search.SearchOpts) ([]search.Paper, error) {
			...
			if opts.Limit != 20 {
				t.Fatalf("expected limit 20 from execution profile, got %d", opts.Limit)
			}
			return []search.Paper{
				{
					ID:       "p1",
...
func TestDeepResearchRouteUsesAutonomousLoopGapLedger(t *testing.T) {
	...
	if capturedLoopReq.MaxSearchTerms != 8 {
		t.Fatalf("expected balanced max search terms 8, got %d", capturedLoopReq.MaxSearchTerms)
	}
	if capturedLoopReq.HitsPerSearch != 20 {
		t.Fatalf("expected balanced hits per search 20, got %d", capturedLoopReq.HitsPerSearch)
	}
...
func TestAutonomousResearchRouteLoopUsesProfileSearchBudget(t *testing.T) {
	...
	if len(searchLimits) == 0 {
		t.Fatalf("expected loop path to search with profile hitsPerSearch=20")
	}
	for _, limit := range searchLimits {
		if limit != 20 {
			t.Fatalf("expected every canonical loop search to use profile hitsPerSearch=20, got %#v", searchLimits)
		}
	}
	...
	if !ok || len(papers) != 20 {
		t.Fatalf("expected loop response to include 20 papers from profile budget, got %#v", payload["papers"])
	}
}
...
func TestAutonomousResearchRouteYoloUsesGuidedStableBalancedProfile(t *testing.T) {
	originalRunUnifiedResearchLoop := runUnifiedResearchLoop
	t.Cleanup(func() {
		runUnifiedResearchLoop = originalRunUnifiedResearchLoop
	})

	var captured wisdev.LoopRequest
	runUnifiedResearchLoop = func(_ context.Context, _ *wisdev.UnifiedResearchRuntime, plane wisdev.ResearchExecutionPlane, req wisdev.LoopRequest, _ func(wisdev.PlanExecutionEvent)) (*wisdev.LoopResult, error) {
		if plane != wisdev.ResearchExecutionPlaneAutonomous {
			t.Fatalf("expected autonomous execution plane, got %q", plane)
		}
		captured = req
		return testResearchLoopResult(req.Query, req.SeedQueries, []search.Paper{
			{ID: "p1", Title: "Sleep memory paper", Source: "crossref"},
		}), nil
	}
	...
	req := httptest.NewRequest(http.MethodPost, "/wisdev/research/autonomous", strings.NewReader(`{
		...
		"session":{"sessionId":"s-yolo","correctedQuery":"sleep and memory","mode":"yolo","serviceTier":"priority"},
		"plan":{"queries":["hippocampal replay","systems consolidation"]}
	}`))
	req = withTestUserID(req, "u1")
	rec := httptest.NewRecorder()
	...
	guided := wisdev.BuildResearchExecutionProfile(context.Background(), "sleep and memory", "guided", "balanced", false, 0)
	if captured.Mode != string(wisdev.WisDevModeYOLO) {
		t.Fatalf("expected yolo mode to be preserved, got %q", captured.Mode)
	}
	if captured.ServiceTier != wisdev.ServiceTierPriority {
		t.Fatalf("expected explicit priority service tier to be preserved, got %q", captured.ServiceTier)
	}
	if captured.MaxSearchTerms != guided.SearchBudget.MaxSearchTerms ||
		captured.HitsPerSearch != guided.SearchBudget.HitsPerSearch ||
		captured.MaxUniquePapers != guided.SearchBudget.MaxUniquePapers ||
		captured.MaxIterations != guided.MaxIterations ||
		captured.AllocatedTokens != guided.AllocatedTokens {
		t.Fatalf("expected yolo route to use guided-stable balanced profile, got req=%+v guided=%+v", captured, guided)
	}
}

func TestDeepResearchRouteYoloUsesGuidedStableBalancedProfile(t *testing.T) {
	originalRunUnifiedResearchLoop := runUnifiedResearchLoop
	t.Cleanup(func() {
		runUnifiedResearchLoop = originalRunUnifiedResearchLoop
	})
	...
	guided := wisdev.BuildResearchExecutionProfile(context.Background(), "sleep and memory", "guided", "balanced", true, 0)
	if captured.Mode != string(wisdev.WisDevModeYOLO) {
		t.Fatalf("expected yolo mode to be preserved, got %q", captured.Mode)
	}
	if captured.ServiceTier != wisdev.ServiceTierPriority {
	...
	if captured.MaxSearchTerms != guided.SearchBudget.MaxSearchTerms ||
		...
		captured.MaxIterations != guided.MaxIterations ||
		captured.AllocatedTokens != guided.AllocatedTokens {
		t.Fatalf("expected yolo deep route to use guided-stable balanced profile, got req=%+v guided=%+v", captured, guided)
	}
}
