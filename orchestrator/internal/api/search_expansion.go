...
func (h *SearchHandler) HandleAggressiveExpansion(w http.ResponseWriter, r *http.Request) {
	...
	var intelligence *search.SearchIntelligence
	if registry := h.resolveRegistry(); registry != nil {
		intelligence = registry.GetIntelligence()
	}

	result := wisdev.GenerateAdaptiveExpansion(
		r.Context(),
		intelligence,
		h.redis,
		query,
		maxVariations,
		includeMeSH,
		req.IncludeAbbreviations,
		req.IncludeTemporal,
		targetAPIs,
	)
	if maxVariations > 0 && len(result.Variations) > maxVariations {
		result.Variations = result.Variations[:maxVariations]
		result.Metadata.TotalVariations = len(result.Variations)
	}
