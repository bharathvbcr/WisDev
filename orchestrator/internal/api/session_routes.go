...
func normalizeRequestedSessionServiceTier(raw string) string {
	switch wisdev.ServiceTier(strings.ToLower(strings.TrimSpace(raw))) {
	case wisdev.ServiceTierStandard, wisdev.ServiceTierFlex, wisdev.ServiceTierPriority:
...
func (s *wisdevServer) registerSessionRoutes(mux *http.ServeMux, agentGateway *wisdev.AgentGateway) {
	...
	handleInitializeSession := func(w http.ResponseWriter, r *http.Request) {
		...
		telemetry.Logger().InfoContext(r.Context(), "session init: query received and validated",
			...
			"requested_mode", strings.TrimSpace(firstNonEmpty(req.Mode, req.ExecutionMode)),
		)
		mode := normalizeSessionMode(firstNonEmpty(req.Mode, req.ExecutionMode))
		requestedServiceTier := normalizeRequestedSessionServiceTier(req.ServiceTier)
		initIdempotencyKey := "wisdev_agent_initialize:" + userID + ":" + string(mode) + ":" + requestedServiceTier + ":" + originalQuery
		if status, cached, ok := enforceIdempotency(r, agentGateway, initIdempotencyKey); ok {
		...
		session = seedYoloAutonomousResearchArtifacts(session, now)
		...
		telemetry.Logger().InfoContext(r.Context(), "session init: session created and persisted",
			...
			"detected_domain", strings.TrimSpace(req.DetectedDomain),
			"mode", string(mode),
			"service_tier", wisdev.AsOptionalString(session["serviceTier"]),
			"question_count", len(questions),
			"complexity_score", complexityScore,
		)
		responseBody := buildAgentQuestioningEnvelopeBody(traceID, session, false)
		body, _ := json.Marshal(responseBody)
		storeIdempotentResponse(agentGateway, r, initIdempotencyKey, body)
	...
	handleCompleteSession := func(w http.ResponseWriter, r *http.Request) {
		...
		telemetry.Logger().InfoContext(r.Context(), "session complete: response prepared",
			...
			"has_autonomous_agenda", session["autonomousAgenda"] != nil,
			"has_memory_tiers", session["memoryTiers"] != nil,
			"has_research_scratchpad", session["researchScratchpad"] != nil,
		)
