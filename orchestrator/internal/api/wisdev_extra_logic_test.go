...
func TestWisDev_MoreHelpers(t *testing.T) {
	...
	t.Run("isAllowedFullPaperCheckpointAction", func(t *testing.T) {
		...
		assert.NoError(t, isAllowedFullPaperCheckpointAction(job, "retrieval", "approve"))

		job["status"] = "completed"
		assert.Error(t, isAllowedFullPaperCheckpointAction(job, "retrieval", "approve"))
	})
