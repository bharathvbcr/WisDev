...
func TestWisDev_AuthHelpers(t *testing.T) {
	...
	t.Run("requireOwnerAccess rejects anonymous owner for anonymous caller", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		rec := httptest.NewRecorder()

		allowed := requireOwnerAccess(rec, req, "anonymous")

		assert.False(t, allowed)
		assert.Equal(t, http.StatusForbidden, rec.Code)
	})

	t.Run("requireOwnerAccess allows internal service for missing owner", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		req = req.WithContext(context.WithValue(req.Context(), contextKey("user_id"), "internal-service"))
		rec := httptest.NewRecorder()

		allowed := requireOwnerAccess(rec, req, "")
	...
	t.Run("resolveAuthorizedUserID", func(t *testing.T) {
		...
		_, err2 := resolveAuthorizedUserID(req, "u2")
		assert.Error(t, err2)
	})

	t.Run("resolveAuthorizedUserID rejects anonymous callers", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		_, err := resolveAuthorizedUserID(req, "u1")
		assert.Error(t, err)
		assert.EqualError(t, err, "authentication required")
	})

	t.Run("resolveAuthorizedUserID allows admin passthrough", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodGet, "/", nil)
		req = req.WithContext(context.WithValue(req.Context(), contextKey("user_id"), "admin"))

		id, err := resolveAuthorizedUserID(req, "u2")
		assert.NoError(t, err)
		assert.Equal(t, "u2", id)
	})
}

func TestWisDev_LogicHelpers(t *testing.T) {
	...
	t.Run("validateStringSlice", func(t *testing.T) {
		...
		assert.Error(t, validateStringSlice([]string{""}, "f", 5, 10))
		assert.Error(t, validateStringSlice([]string{"\t"}, "f", 5, 10))
	})
...
func TestWisDev_Concurrency(t *testing.T) {
