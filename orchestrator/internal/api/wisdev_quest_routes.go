...
func (s *wisdevServer) registerQuestRoutes(mux *http.ServeMux, agentGateway *wisdev.AgentGateway) {
	...
	mux.HandleFunc("/wisdev/quests/", func(w http.ResponseWriter, r *http.Request) {
		...
		case "preflight":
			...
			checkedQuest, err := runtime.RunReferenceDoctorPreflight(r.Context(), questID)
			if err != nil {
				WriteError(w, http.StatusInternalServerError, ErrInternal, "failed to run quest preflight", map[string]any{"error": err.Error(), "questId": questID})
				return
			}
			traceID := writeEnvelope(w, "quest", checkedQuest)
			s.journalEvent("quest_preflight_checked", "/wisdev/quests/"+questID+"/preflight", traceID, checkedQuest.SessionID, checkedQuest.UserID, checkedQuest.QuestID, "", "Quest doctor preflight checked.", map[string]any{"questId": questID}, map[string]any{
				"ready": checkedQuest.ReferenceInspiration != nil && checkedQuest.ReferenceInspiration.DoctorPreflight.Ready,
			})
		default:
