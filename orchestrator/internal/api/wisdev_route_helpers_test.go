...
func TestWisDev_CommitteeHelpers(t *testing.T) {
	...
	t.Run("buildCommitteePapers", func(t *testing.T) {
		...
		assert.Equal(t, true, mapped[0]["relevanceChecked"])
		assert.Equal(t, float64(93), mapped[0]["relevanceScore"])
		assert.Contains(t, mapped[0]["relevanceReason"], "Go-ranked relevance estimate")
		assert.Equal(t, 1, mapped[0]["qualityTier"])
		explanation := mapped[0]["rankingExplanation"].(map[string]any)
		assert.Equal(t, float64(93), explanation["finalScore"])
	})

	t.Run("deepResearchPayloadOrganizationLogAttrs includes relevance audit fields", func(t *testing.T) {
					...
					"sources": []map[string]any{
						{
							...
							"relevanceScore":     float64(93),
							"rankingExplanation": map[string]any{"finalScore": float64(93)},
						},
						{
							"id":             "p2",
							"relevanceScore": float64(72),
						},
					},
		...
		attrs := deepResearchPayloadOrganizationLogAttrs(payload)
		...
		assert.Equal(t, 2, attrMap["category_source_count"])
		assert.Equal(t, 1, attrMap["assignment_meta_category_count"])
		assert.Equal(t, 1, attrMap["relevance_checked_source_count"])
		assert.Equal(t, 2, attrMap["relevance_scored_source_count"])
		assert.Equal(t, 1, attrMap["ranking_explanation_source_count"])
		assert.Equal(t, float64(72), attrMap["min_relevance_score"])
		assert.Equal(t, float64(93), attrMap["max_relevance_score"])
		assert.Equal(t, float64(82.5), attrMap["avg_relevance_score"])
	})
...
func TestWisDev_AgentHelpers(t *testing.T) {
	...
	t.Run("seedYoloAutonomousResearchArtifacts creates independent research plan", func(t *testing.T) {
		session := map[string]any{
			"sessionId":      "ws_yolo_seed",
			"userId":         "u1",
			"query":          "RLHF safety",
			"originalQuery":  "RLHF safety",
			"correctedQuery": "RLHF safety",
		...
		seeded := seedYoloAutonomousResearchArtifacts(session, 1234)
		plan := mapAny(seeded["orchestrationPlan"])
		queries := sliceStrings(plan["queries"])
		assert.Equal(t, true, plan["generatedFromAutonomy"])
		assert.Equal(t, "tree_of_hypotheses", wisdev.AsOptionalString(plan["planningStrategy"]))
		assert.Greater(t, len(queries), 3)
		assert.Contains(t, queries[0], "RLHF safety")
		assert.Len(t, sliceAnyMap(plan["selfReview"]), 3)
		assert.Len(t, sliceStrings(plan["stopCriteria"]), 3)
		assert.Equal(t, "autonomous_yolo_no_user_input", wisdev.AsOptionalString(seeded["questionStopReason"]))
		assert.Empty(t, sliceAnyMap(seeded["questions"]))

		topicTree := mapAny(seeded["topicTree"])
		rootNode := mapAny(topicTree["rootNode"])
		assert.Equal(t, "RLHF safety", wisdev.AsOptionalString(rootNode["label"]))
		assert.Greater(t, len(sliceAnyMap(rootNode["children"])), 3)

		hypotheses := sliceAnyMap(seeded["hypotheses"])
		assert.Greater(t, len(hypotheses), 3)
		assert.Contains(t, wisdev.AsOptionalString(hypotheses[0]["claim"]), "RLHF safety")

		graph := mapAny(seeded["reasoningGraph"])
		assert.Equal(t, "yolo_root", wisdev.AsOptionalString(graph["root"]))
		assert.Greater(t, len(sliceAnyMap(graph["nodes"])), 3)
		assert.Contains(t, fmt.Sprint(graph["nodes"]), "self_review_checkpoint")

		agenda := mapAny(seeded["autonomousAgenda"])
		assert.Equal(t, false, agenda["requiresUserInput"])
		assert.Greater(t, len(sliceAnyMap(agenda["branchItems"])), 3)
		assert.Contains(t, sliceStrings(agenda["operatingPrinciples"]), "Do not ask the user for clarification during YOLO execution.")

		memoryTiers := mapAny(seeded["memoryTiers"])
		assert.Greater(t, len(sliceAnyMap(memoryTiers["shortTermWorking"])), 3)
		assert.NotEmpty(t, sliceAnyMap(memoryTiers["artifactMemory"]))
		scratchpad := mapStringString(seeded["researchScratchpad"])
		assert.Contains(t, scratchpad["selfReviewProtocol"], "evidence sufficiency")

		canonical := buildCanonicalAgentSession(seeded)
		require.NotNil(t, canonical)
		require.NotNil(t, canonical.Plan)
		assert.Equal(t, wisdev.WisDevModeYOLO, canonical.Mode)
		assert.Greater(t, len(canonical.Plan.Steps), 5)
		assert.Greater(t, len(canonical.Hypotheses), 3)
		require.NotNil(t, canonical.ReasoningGraph)
		assert.Greater(t, len(canonical.ReasoningGraph.Nodes), 3)
		require.NotNil(t, canonical.MemoryTiers)
		assert.Greater(t, len(canonical.MemoryTiers.ShortTermWorking), 3)
		assert.NotEmpty(t, canonical.ResearchScratchpad["handoffContract"])
	})

	t.Run("seedYoloAutonomousResearchArtifacts leaves guided sessions alone", func(t *testing.T) {
		session := map[string]any{
			"sessionId":      "ws_guided_seed",
			"correctedQuery": "guided query",
			"mode":           "guided",
		}
		seeded := seedYoloAutonomousResearchArtifacts(session, 1234)
		assert.Nil(t, seeded["orchestrationPlan"])
		assert.Nil(t, seeded["topicTree"])
		assert.Nil(t, seeded["hypotheses"])
	})
	...
	t.Run("buildDeepResearchPayload", func(t *testing.T) {
		...
		paperPools := res["paperPools"].([]map[string]any)
		assert.Equal(t, "cat1", paperPools[0]["category"])
		assert.Equal(t, 1, paperPools[0]["totalAvailable"])
		assert.Len(t, paperPools[0]["papers"], 1)
	})

	t.Run("buildDeepResearchLoopPayload categorizes papers from query coverage", func(t *testing.T) {
		shared := search.Paper{ID: "shared", Title: "Shared paper", Link: "https://example.com/shared", Score: 0.9}
		bench := search.Paper{ID: "bench", Title: "Benchmark paper", Link: "https://example.com/bench", Score: 0.8}
		ablation := search.Paper{ID: "ablation", Title: "Ablation paper", Link: "https://example.com/ablation", Score: 0.7}

		res := buildDeepResearchLoopPayload("rlhf", []string{"cs"}, "cs", &wisdev.LoopResult{
			Papers:          []search.Paper{shared, bench, ablation},
			ExecutedQueries: []string{"rlhf benchmarks", "rlhf ablations"},
			QueryCoverage: map[string][]search.Paper{
				"rlhf benchmarks": {shared, bench},
				"rlhf ablations":  {shared, ablation},
			},
		})

		categorized := res["categorizedSources"].([]map[string]any)
		require.Len(t, categorized, 2)
		assert.Equal(t, "rlhf benchmarks", categorized[0]["category"])
		assert.Equal(t, "rlhf ablations", categorized[1]["category"])

		benchSources := categorized[0]["sources"].([]map[string]any)
		ablationSources := categorized[1]["sources"].([]map[string]any)
		require.Len(t, benchSources, 2)
		require.Len(t, ablationSources, 1)
		assert.Equal(t, "shared", benchSources[0]["id"])
		assert.Equal(t, "bench", benchSources[1]["id"])
		assert.Equal(t, true, benchSources[0]["relevanceChecked"])
		assert.GreaterOrEqual(t, benchSources[0]["relevanceScore"].(float64), float64(80))
		benchMeta := categorized[0]["assignmentMeta"].(map[string]any)
		assert.Equal(t, "go_query_coverage", benchMeta["assignmentMethod"])
		assert.Equal(t, "executed_query", benchMeta["assignmentScope"])
		assert.Equal(t, "rlhf benchmarks", benchMeta["query"])
		assert.Equal(t, 2, benchMeta["rawCoverageCount"])
		assert.Equal(t, 2, benchMeta["acceptedUniqueCount"])
		assert.Equal(t, 0, benchMeta["duplicateCount"])
		assert.Equal(t, "ablation", ablationSources[0]["id"])
		ablationMeta := categorized[1]["assignmentMeta"].(map[string]any)
		assert.Equal(t, 2, ablationMeta["rawCoverageCount"])
		assert.Equal(t, 1, ablationMeta["acceptedUniqueCount"])
		assert.Equal(t, 1, ablationMeta["duplicateCount"])
		assert.Equal(t, []string{"rlhf benchmarks", "rlhf ablations"}, res["categories"])
		trace := res["searchRunTrace"].(map[string]any)
		assert.Equal(t, "wisdev.searchRunTrace.v1", trace["schema"])
		assert.Equal(t, 2, trace["executedQueryCount"])
		assert.Equal(t, 2, trace["coverageQueryCount"])
		assert.Equal(t, 3, trace["retrievedPaperCount"])
		assert.Equal(t, 4, trace["coveragePaperCount"])
		assert.Equal(t, 3, trace["uniqueCoverageCount"])
		assert.Equal(t, 1, trace["duplicatePaperCount"])
		tabs := trace["categorizedTabs"].([]map[string]any)
		require.Len(t, tabs, 2)
		assert.Equal(t, "rlhf benchmarks", tabs[0]["query"])
		assert.Equal(t, 2, tabs[0]["acceptedUniqueCount"])
		assert.Equal(t, "rlhf ablations", tabs[1]["query"])
		assert.Equal(t, 1, tabs[1]["acceptedUniqueCount"])
	})

	t.Run("buildDeepResearchLoopPayload splits broad query coverage into evidence categories", func(t *testing.T) {
		rootPrompt := "What are the new AI architectures? Are there any training advancements?"
		res := buildDeepResearchLoopPayload(rootPrompt, []string{"cs"}, "cs", &wisdev.LoopResult{
			Papers: []search.Paper{
				{ID: "survey", Title: "Survey of new AI architectures", Abstract: "A review and taxonomy of modern model families.", Link: "https://example.com/survey", Score: 0.9},
				{ID: "training", Title: "Training advances for large language models", Abstract: "Curriculum optimization and alignment methods.", Link: "https://example.com/training", Score: 0.8},
				{ID: "inference", Title: "Speculative decoding for faster inference", Abstract: "Serving latency and throughput improvements.", Link: "https://example.com/inference", Score: 0.7},
			},
			ExecutedQueries: []string{rootPrompt},
			QueryCoverage: map[string][]search.Paper{
				rootPrompt: {
					{ID: "survey", Title: "Survey of new AI architectures", Abstract: "A review and taxonomy of modern model families.", Link: "https://example.com/survey", Score: 0.9},
					{ID: "training", Title: "Training advances for large language models", Abstract: "Curriculum optimization and alignment methods.", Link: "https://example.com/training", Score: 0.8},
		...
		labels := []string{
			categorized[0]["category"].(string),
			categorized[1]["category"].(string),
			categorized[2]["category"].(string),
		}
		assert.Equal(t, []string{"Introduction and surveys", "Training advances", "Inference advances"}, labels)
		assert.NotContains(t, labels, rootPrompt)
		assert.Equal(t, []string{"Introduction and surveys", "Training advances", "Inference advances"}, res["categories"])
		meta := categorized[0]["assignmentMeta"].(map[string]any)
		assert.Equal(t, rootPrompt, meta["query"])
		assert.Equal(t, "Introduction and surveys", meta["displayCategory"])
		assert.Equal(t, 1, meta["acceptedUniqueCount"])
	})
}
