...
func TestWisDevResearchRoutes_Autonomous_LiveLoopDegradedFallback(t *testing.T) {
	...
	runUnifiedResearchLoop = func(
		...
		assert.Equal(t, wisdev.ResearchExecutionPlaneAutonomous, plane)
		assert.Equal(t, "sleep and memory", req.Query)
		return &wisdev.LoopResult{
			FinalAnswer:     "Fallback loop synthesis",
			Papers:          []internalsearch.Paper{{ID: "paper-1", Title: "Fallback sleep and memory", Abstract: "Fallback abstract", Source: "crossref"}},
			ExecutedQueries: []string{"sleep and memory"},
			Iterations:      1,
			Converged:       true,
		}, nil
	}
