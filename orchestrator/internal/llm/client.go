...
const recoverableStructuredMinSpacing = 4 * time.Second
...
func IsProviderRateLimitError(err error) bool {
	...
	return strings.Contains(raw, "rate limited") ||
		...
		containsHTTPStatus429Signal(raw)
}

func containsHTTPStatus429Signal(raw string) bool {
	switch {
	case raw == "429":
		return true
	case strings.HasPrefix(raw, "429 "),
		strings.HasPrefix(raw, "429,"),
		strings.HasPrefix(raw, "429:"),
		strings.Contains(raw, " error 429"),
		strings.Contains(raw, " status 429"),
		strings.Contains(raw, " status: 429"),
		strings.Contains(raw, " status=429"),
		strings.Contains(raw, " code 429"),
		strings.Contains(raw, " code: 429"),
		strings.Contains(raw, " code=429"),
...
func shouldFallbackStructuredOutputToSidecar(err error) bool {
	...
	return strings.Contains(message, "parameter is not supported in vertex ai") ||
		...
		strings.Contains(message, "structured output is not valid json") ||
		strings.Contains(message, "structured output returned empty text") ||
...
func isRecoverableStructuredRequest(req *llmpb.StructuredRequest) bool {
	...
	case "structured_high_value":
		return tier == "" || tier == "standard" || tier == "priority"
	default:
		return false
	}
}
	...
	if c.hasDirectStructuredProvider() && !c.useHTTPTransport() {
		...
		if err != nil && shouldFallbackStructuredOutputToSidecar(err) {
			slog.Warn("llm structured output direct compatibility fallback",
				...
				"error", err.Error(),
				"reason", "vertex_direct_recoverable_structured_output",
			)
...
// Uses the token-aware internal helper to report accurate billing counters.
