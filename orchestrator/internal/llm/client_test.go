...
func TestStructuredOutputDirect(t *testing.T) {
	...
	t.Run("falls back to sidecar when vertex direct returns malformed JSON", func(t *testing.T) {
		mockModels := new(mockGenAIModels)
		sidecar := &mockLLMServiceClient{}
		vc := &VertexClient{client: mockModels}
		c := NewClient()
		...
		mockModels.On("GenerateContent", mock.Anything, mock.Anything, mock.Anything, mock.Anything).
			Return(&genai.GenerateContentResponse{
				Candidates: []*genai.Candidate{
					{Content: &genai.Content{Parts: []*genai.Part{{Text: `{"suggestedDomains":`}}}},
				},
			}, nil).Once()
		sidecar.On("StructuredOutput", mock.Anything, req).
			Return(&llmv1.StructuredResponse{JsonResult: jsonResp, SchemaValid: true}, nil).Once()
...
func TestShouldFallbackStructuredOutputToSidecar(t *testing.T) {
	...
	t.Run("matches malformed direct structured output", func(t *testing.T) {
		assert.True(t, shouldFallbackStructuredOutputToSidecar(errors.New("gemini structured output is not valid JSON: {\"tasks\":")))
		assert.True(t, shouldFallbackStructuredOutputToSidecar(errors.New("gemini structured output returned empty text")))
		assert.True(t, shouldFallbackStructuredOutputToSidecar(errors.New("no structured candidates returned (model may have blocked output)")))
	})
