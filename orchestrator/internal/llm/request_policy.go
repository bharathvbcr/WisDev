...
const (
	...
	minStructuredHighValueLatencyBudgetMs = 30_000
...
func ResolveRequestPolicy(input RequestPolicyInput) RequestPolicy {
	...
	latencyBudgetMs := latencyBudgetForClass(rawBudget, class)
...
func latencyBudgetForClass(value int, class RequestClass) int {
	budget := clampLatencyBudgetMs(value)
	if class == RequestClassStructuredHighValue && budget < minStructuredHighValueLatencyBudgetMs {
		return minStructuredHighValueLatencyBudgetMs
	}
	return budget
}
