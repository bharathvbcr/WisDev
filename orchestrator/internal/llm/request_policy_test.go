...
func TestResolveRequestPolicy(t *testing.T) {
	...
	t.Run("structured high value uses dynamic thinking and priority tier", func(t *testing.T) {
		...
		if policy.LatencyBudgetMs != minStructuredHighValueLatencyBudgetMs {
			t.Fatalf("expected structured budget floor %d, got %d", minStructuredHighValueLatencyBudgetMs, policy.LatencyBudgetMs)
		}
	})
