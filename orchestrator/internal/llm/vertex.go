...
// FileSearchGenerationRequest configures a grounded Gemini API File Search
// generation call. File Search is a Gemini Developer API feature, so
// Vertex-backed clients use a Secret Manager/API-key Gemini fallback.
type FileSearchGenerationRequest struct {
	ModelID        string
	Prompt         string
	SystemPrompt   string
	StoreNames     []string
...
// FileSearchGenerationResponse is the grounded answer and attribution returned
// by Gemini API File Search.
type FileSearchGenerationResponse struct {
	Text             string
...
// FileSearchCitation captures page, media, and metadata attribution from
// Gemini grounding metadata without binding the llm package to the RAG package.
type FileSearchCitation struct {
	Claim           string
	SourceID        string
	SourceTitle     string
	SourceURI       string
...
// Google's Standard PayGo guidance recommends smoothing sharp second-level
// spikes; keep per-process generation single-flight and let higher-level search
// fan-out provide parallelism outside the model provider.
const vertexGenerateContentMaxConcurrency = 1
...
func NewVertexClient(ctx context.Context, projectID, location string) (*VertexClient, error) {
	...
	location = ResolveGoogleCloudLocation(location)
...
func ResolveGoogleCloudLocation(location string) string {
	...
	if location := strings.TrimSpace(os.Getenv("GOOGLE_CLOUD_REGION")); location != "" {
		return location
	}
	return "global"
}

// NewGeminiAPIFileSearchClient creates a Gemini Developer API client for File
// Search. The feature is not supported by the Vertex AI backend in the GenAI
// SDK, so this path deliberately resolves a server-side API key through the
// existing secret-management chain.
func NewGeminiAPIFileSearchClient(ctx context.Context, projectID string) (*VertexClient, error) {
	projectID = strings.TrimSpace(projectID)
	if projectID == "" {
		projectID, _ = resolveProjectID(ctx)
	}
	apiKey, apiKeySource, err := ResolveGoogleAPIKey(ctx, projectID)
	if err != nil {
		return nil, fmt.Errorf("failed to resolve Gemini API key for file search: %w", err)
	}
	if apiKey == "" {
		return nil, fmt.Errorf("failed to create Gemini file search client: GOOGLE_API_KEY or GEMINI_API_KEY is required")
	}
	client, err := newModelsClient(ctx, &genai.ClientConfig{
		APIKey:  apiKey,
		Backend: genai.BackendGeminiAPI,
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create Gemini file search client: %w", err)
	}
	return &VertexClient{
		client:           client,
...
func (v *VertexClient) fileSearchClient(ctx context.Context) (*VertexClient, error) {
	if v != nil && v.client != nil && strings.HasPrefix(v.BackendName(), "gemini_api") {
		return v, nil
	}
	return NewGeminiAPIFileSearchClient(ctx, "")
}

// GenerateWithFileSearch answers a prompt using Gemini API File Search stores
// and returns grounding metadata as typed citations.
func (v *VertexClient) GenerateWithFileSearch(ctx context.Context, req FileSearchGenerationRequest) (*FileSearchGenerationResponse, error) {
	storeNames := normalizedFileSearchStoreNames(req.StoreNames)
	if len(storeNames) == 0 {
		return nil, fmt.Errorf("file search store names are required")
	}
	prompt := strings.TrimSpace(req.Prompt)
	if prompt == "" {
		return nil, fmt.Errorf("file search prompt is required")
	}

	client, err := v.fileSearchClient(ctx)
	if err != nil {
		return nil, err
	}
	if client == nil || client.client == nil {
		return nil, fmt.Errorf("gemini file search client is not configured")
	}

	modelID := strings.TrimSpace(req.ModelID)
	if modelID == "" {
		modelID = ResolveLightModel()
	}
	...
	if filter := strings.TrimSpace(req.MetadataFilter); filter != "" {
	...
	slog.Info("gemini file search generation start",
		"component", "llm.vertex",
		"operation", "generate_file_search",
		"stage", "request_start",
		"model", modelID,
		"store_count", len(storeNames),
		"metadata_filter_set", strings.TrimSpace(req.MetadataFilter) != "",
		"top_k", req.TopK,
	...
	func() {
		defer releaseSlot()
		resp, err = client.client.GenerateContent(ctx, modelID, contents, config)
	}()
	if err != nil {
		slog.Warn("gemini file search generation failed",
			"component", "llm.vertex",
			"operation", "generate_file_search",
		...
		return nil, fmt.Errorf("file search generate content failed: %w", err)
	}
	text, candidate, err := firstCandidateText(resp)
	if err != nil {
		return nil, err
	}
	out := &FileSearchGenerationResponse{
		Text:             text,
		Citations:        extractFileSearchCitations(candidate),
		Backend:          client.BackendName(),
		CredentialSource: client.CredentialSource(),
	}
	if candidate != nil && candidate.GroundingMetadata != nil {
		out.RetrievalQueries = append([]string(nil), candidate.GroundingMetadata.RetrievalQueries...)
	}
	slog.Info("gemini file search generation success",
		"component", "llm.vertex",
		"operation", "generate_file_search",
...
func normalizedFileSearchStoreNames(storeNames []string) []string {
	seen := make(map[string]struct{}, len(storeNames))
	out := make([]string, 0, len(storeNames))
...
func firstCandidateText(resp *genai.GenerateContentResponse) (string, *genai.Candidate, error) {
	if resp == nil || len(resp.Candidates) == 0 {
		return "", nil, fmt.Errorf("no candidates returned (model may have blocked output)")
	}
	candidate := resp.Candidates[0]
	if candidate == nil || candidate.Content == nil || len(candidate.Content.Parts) == 0 {
		return "", candidate, fmt.Errorf("no candidate content returned")
	}
	...
	if text == "" {
		return "", candidate, fmt.Errorf("gemini file search returned empty text")
	}
	return text, candidate, nil
}

func extractFileSearchCitations(candidate *genai.Candidate) []FileSearchCitation {
	if candidate == nil || candidate.GroundingMetadata == nil {
		return nil
	}
	metadata := candidate.GroundingMetadata
	citations := make([]FileSearchCitation, 0, len(metadata.GroundingChunks))
	...
	if len(metadata.GroundingSupports) > 0 {
		for _, support := range metadata.GroundingSupports {
			...
			if support.Segment != nil {
				claim = strings.TrimSpace(support.Segment.Text)
			}
			for position, rawIdx := range support.GroundingChunkIndices {
				idx := int(rawIdx)
				if idx < 0 || idx >= len(metadata.GroundingChunks) {
					continue
				}
				citation := fileSearchCitationFromChunk(metadata.GroundingChunks[idx], claim, confidenceForSupport(support, position), idx)
				if citation.SourceID == "" && citation.SourceURI == "" && citation.SourceTitle == "" && citation.SourceText == "" && citation.MediaURI == "" {
					continue
				}
				key := fileSearchCitationKey(citation)
				if _, ok := seen[key]; ok {
					continue
				}
				seen[key] = struct{}{}
	...
	for idx, chunk := range metadata.GroundingChunks {
		citation := fileSearchCitationFromChunk(chunk, "", 0, idx)
		key := fileSearchCitationKey(citation)
		if key == "" {
			continue
		}
...
func fileSearchCitationFromChunk(chunk *genai.GroundingChunk, claim string, confidence float64, idx int) FileSearchCitation {
	citation := FileSearchCitation{
		Claim:      strings.TrimSpace(claim),
		Confidence: confidence,
		SourceKind: "file_search",
	}
	if chunk == nil {
		return citation
	}
	if retrieved := chunk.RetrievedContext; retrieved != nil {
		citation.SourceTitle = strings.TrimSpace(retrieved.Title)
		citation.SourceURI = strings.TrimSpace(retrieved.URI)
		citation.SourceText = strings.TrimSpace(retrieved.Text)
		citation.FileSearchStore = strings.TrimSpace(retrieved.FileSearchStore)
		citation.CustomMetadata = flattenGroundingMetadata(retrieved.CustomMetadata)
		if retrieved.RAGChunk != nil && retrieved.RAGChunk.PageSpan != nil {
			citation.PageStart = int(retrieved.RAGChunk.PageSpan.FirstPage)
			citation.PageEnd = int(retrieved.RAGChunk.PageSpan.LastPage)
		}
		citation.SourceID = firstNonBlank(retrieved.URI, retrieved.Title, fmt.Sprintf("file_search_chunk_%d", idx+1))
		return citation
	}
	if image := chunk.Image; image != nil {
		citation.SourceKind = "file_search_image"
		citation.SourceTitle = strings.TrimSpace(image.Title)
		citation.SourceURI = strings.TrimSpace(image.SourceURI)
		citation.MediaURI = strings.TrimSpace(image.ImageURI)
		citation.MediaID = firstNonBlank(image.ImageURI, image.SourceURI, fmt.Sprintf("file_search_image_%d", idx+1))
		citation.SourceID = citation.MediaID
		return citation
	}
	return citation
}

func confidenceForSupport(support *genai.GroundingSupport, position int) float64 {
	if support == nil || position < 0 || position >= len(support.ConfidenceScores) {
		return 0
	}
	score := support.ConfidenceScores[position]
	if score < 0 {
		return 0
	}
	if score > 1 {
...
func flattenGroundingMetadata(items []*genai.GroundingChunkCustomMetadata) map[string]any {
	if len(items) == 0 {
		return nil
	}
	...
	for _, item := range items {
		...
		switch {
		case item.StringListValue != nil:
			out[key] = append([]string(nil), item.StringListValue.Values...)
		case item.NumericValue != nil:
			out[key] = float64(*item.NumericValue)
		case strings.TrimSpace(item.StringValue) != "":
			out[key] = strings.TrimSpace(item.StringValue)
		}
	}
	if len(out) == 0 {
		return nil
...
func fileSearchCitationKey(c FileSearchCitation) string {
	return strings.Join([]string{
		strings.TrimSpace(c.Claim),
		strings.TrimSpace(c.SourceID),
		strings.TrimSpace(c.SourceURI),
		strings.TrimSpace(c.MediaURI),
		fmt.Sprint(c.PageStart),
		fmt.Sprint(c.PageEnd),
	}, "|")
}
...
func (v *VertexClient) GenerateText(ctx context.Context, modelID, prompt, systemPrompt string, temperature float32, maxTokens int32) (string, error) {
	...
	startedAt := time.Now()
	attempt := 0
	resp, err := runVertexGenkitModelMiddleware(ctx, vertexGenkitModelCallOptions{
		...
		InitialDelay:  vertexRetryDelay(""),
		MaxDelay:      800 * time.Millisecond,
		BackoffFactor: 2,
		NoJitter:      true,
	}, func(callCtx context.Context) (*genai.GenerateContentResponse, error) {
		if callCtx.Err() != nil {
		...
		releaseSlot, limiterWait, limiterErr := acquireVertexGenerateContentSlot(callCtx)
		if limiterErr != nil {
			return nil, limiterErr
		}
		if remaining := vertexProviderRateLimitRemaining(time.Now()); remaining > 0 {
		...
		func() {
			defer releaseSlot()
			resp, err = v.client.GenerateContent(callCtx, modelID, contents, config)
		}()
		attemptLatencyMs := time.Since(attemptStart).Milliseconds()
		if err != nil {
			callErr := fmt.Errorf("generate content failed: %w", err)
		...
		if resp == nil {
			slog.Warn("vertex text generation nil response",
				"component", "llm.vertex",
				"operation", "generate_text",
				"stage", "nil_response",
				"model", modelID,
			...
			return nil, fmt.Errorf("vertex text generation returned nil response")
		}
		...
		text := strings.TrimSpace(textBuilder.String())
		if text == "" {
			return nil, fmt.Errorf("vertex text generation returned empty text")
		}
		return resp, nil
	})
	if err != nil {
	...
	if len(resp.Candidates) == 0 || resp.Candidates[0].Content == nil {
		return "", fmt.Errorf("no candidates returned (model may have blocked output)")
	}
	var textBuilder strings.Builder
	for _, part := range resp.Candidates[0].Content.Parts {
		if part != nil {
	...
	text := strings.TrimSpace(textBuilder.String())
	if text == "" {
		return "", fmt.Errorf("vertex text generation returned empty text")
	}
	return text, nil
}
...
func (v *VertexClient) EmbedText(ctx context.Context, modelID, text, taskType string) ([]float32, error) {
	if modelID == "" {
		modelID = ResolveEmbeddingModel("standard")
	}
...
func structuredThinkingConfigForModel(modelID string, thinkingBudget *int32, maxOutputTokens int32) *genai.ThinkingConfig {
	effectiveBudget := thinkingBudget
	if shouldMinimizeStructuredThinking(modelID, thinkingBudget, maxOutputTokens) {
		zeroBudget := int32(0)
		effectiveBudget = &zeroBudget
	}
	return thinkingConfigForModel(modelID, effectiveBudget)
}
...
func shouldMinimizeStructuredThinking(modelID string, thinkingBudget *int32, maxOutputTokens int32) bool {
	...
	normalized := strings.ToLower(strings.TrimSpace(modelID))
	return strings.HasPrefix(normalized, "gemini-2.5-flash") || strings.HasPrefix(normalized, "gemini-3")
}
...
func (v *VertexClient) generateStructuredWithTokens(ctx context.Context, modelID, prompt, systemPrompt string, jsonSchemaStr string, temperature float32, maxTokens int32, serviceTier string, thinkingBudget *int32, requestClass string, retryProfile string) (result string, inputTokens, outputTokens int32, err error) {
	...
	if thinkingConfig := structuredThinkingConfigForModel(modelID, thinkingBudget, maxTokens); thinkingConfig != nil {
	...
	type structuredMiddlewareResult struct {
		text      string
		inTokens  int32
		outTokens int32
	}
	...
	startedAt := time.Now()
	attempt := 0
	structuredResult, err := runVertexGenkitModelMiddleware(ctx, vertexGenkitModelCallOptions{
		...
		BackoffFactor: 2,
		NoJitter:      true,
	}, func(callCtx context.Context) (structuredMiddlewareResult, error) {
		if callCtx.Err() != nil {
		...
		if remaining := vertexProviderRateLimitRemaining(time.Now()); remaining > 0 {
			...
			return structuredMiddlewareResult{}, err
		}
		attemptStart := time.Now()
		...
		if hasDeadline {
			...
			if remainingMs <= 0 {
				if ctxErr := callCtx.Err(); ctxErr != nil {
					return structuredMiddlewareResult{}, ctxErr
				}
				return structuredMiddlewareResult{}, context.DeadlineExceeded
			}
		}
		...
		func() {
			defer releaseSlot()
			resp, callErr = v.client.GenerateContent(callCtx, modelID, contents, config)
		}()
		attemptLatencyMs := time.Since(attemptStart).Milliseconds()
		if callErr != nil {
			err := fmt.Errorf("generate structured content failed: %w", callErr)
		...
		if resp == nil {
			err := fmt.Errorf("vertex structured output returned nil response")
			slog.Warn("vertex structured output nil response",
				"component", "llm.vertex",
				"operation", "structured_output",
				"stage", "nil_response",
				"model", modelID,
		...
		if len(resp.Candidates) == 0 || resp.Candidates[0].Content == nil || len(resp.Candidates[0].Content.Parts) == 0 {
			err := fmt.Errorf("no structured candidates returned (model may have blocked output)")
		...
		if text == "" {
			return structuredMiddlewareResult{}, fmt.Errorf("gemini structured output returned empty text")
		}
		if !json.Valid([]byte(text)) {
			return structuredMiddlewareResult{}, fmt.Errorf("gemini structured output is not valid JSON: %.200s", text)
		}
		...
		return structuredMiddlewareResult{text: text, inTokens: inTok, outTokens: outTok}, nil
	})
	if err != nil {
		logVertexStructuredOutputExhausted(modelID, maxAttempts, time.Since(startedAt), err, v.backend)
		return "", 0, 0, err
	}
	return structuredResult.text, structuredResult.inTokens, structuredResult.outTokens, nil
}

func logVertexStructuredOutputExhausted(modelID string, maxAttempts int, totalLatency time.Duration, lastErr error, backend string) {
	errorClass := classifyVertexError(lastErr)
	lastError := ""
	if lastErr != nil {
	...
	if isRetryableVertexErrorClass(errorClass) {
		slog.Warn("vertex structured output exhausted all attempts; fallback expected", attrs...)
		return
	}
	slog.Error("vertex structured output exhausted all attempts", attrs...)
}
...
func classifyVertexError(err error) string {
	...
	case strings.Contains(msg, "499") || strings.Contains(msg, "cancelled") || strings.Contains(msg, "canceled"):
		return "canceled"
