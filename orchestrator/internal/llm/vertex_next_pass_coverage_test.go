...
func TestNewVertexClientLocationDefaultsToGlobalWhenEmpty(t *testing.T) {
...
func TestNewVertexClientLocationUsesEnvironmentWhenEmpty(t *testing.T) {
	...
	assert.NotNil(t, client)
	if assert.NotNil(t, capturedConfig) {
		assert.Equal(t, "europe-west1", capturedConfig.Location)
	}
}

func TestVertexGenerateContentConcurrencyIsSmoothedForPayGo(t *testing.T) {
	assert.Equal(t, 1, vertexGenerateContentMaxConcurrency)
	assert.Equal(t, vertexGenerateContentMaxConcurrency, cap(vertexGenerateContentSlots))
}
...
func TestStructuredThinkingConfigForModelTinyOutputBudget(t *testing.T) {
	t.Run("gemini 2.5 flash nil budget disables thinking", func(t *testing.T) {
		cfg := structuredThinkingConfigForModel("gemini-2.5-flash-lite", nil, 64)
		require.NotNil(t, cfg)
		require.NotNil(t, cfg.ThinkingBudget)
		assert.Equal(t, int32(0), *cfg.ThinkingBudget)
	})

	t.Run("gemini 2.5 flash dynamic budget disables thinking", func(t *testing.T) {
		budget := int32(-1)
		cfg := structuredThinkingConfigForModel("gemini-2.5-flash", &budget, 64)
		require.NotNil(t, cfg)
		require.NotNil(t, cfg.ThinkingBudget)
		assert.Equal(t, int32(0), *cfg.ThinkingBudget)
	})

	t.Run("larger output budget preserves dynamic thinking", func(t *testing.T) {
		budget := int32(-1)
		cfg := structuredThinkingConfigForModel("gemini-2.5-flash", &budget, 512)
		require.NotNil(t, cfg)
		require.NotNil(t, cfg.ThinkingBudget)
		assert.Equal(t, int32(-1), *cfg.ThinkingBudget)
	})

	t.Run("explicit positive budget is preserved", func(t *testing.T) {
		budget := int32(1024)
		cfg := structuredThinkingConfigForModel("gemini-2.5-flash", &budget, 64)
		require.NotNil(t, cfg)
		require.NotNil(t, cfg.ThinkingBudget)
		assert.Equal(t, int32(1024), *cfg.ThinkingBudget)
	})

	t.Run("gemini 2.5 pro is not disabled", func(t *testing.T) {
		budget := int32(-1)
		cfg := structuredThinkingConfigForModel("gemini-2.5-pro", &budget, 64)
		require.NotNil(t, cfg)
		require.NotNil(t, cfg.ThinkingBudget)
		assert.Equal(t, int32(-1), *cfg.ThinkingBudget)
	})

	t.Run("gemini 3 dynamic budget uses minimal thinking", func(t *testing.T) {
		budget := int32(-1)
		cfg := structuredThinkingConfigForModel("gemini-3-flash", &budget, 64)
		require.NotNil(t, cfg)
		assert.Equal(t, genai.ThinkingLevelMinimal, cfg.ThinkingLevel)
		assert.Nil(t, cfg.ThinkingBudget)
	})
...
func TestLogVertexStructuredOutputExhaustedDowngradesProviderBackpressure(t *testing.T) {
	...
	logVertexStructuredOutputExhausted(
		"gemini-test",
		2,
		1500*time.Millisecond,
		errors.New("Error 429, Message: Resource exhausted"),
		"vertex_ai",
	)

	logText := logBuffer.String()
	assert.Contains(t, logText, "level=WARN")
	assert.Contains(t, logText, "error_class=rate_limit")
	assert.Contains(t, logText, "fallback expected")
	assert.NotContains(t, strings.ToUpper(logText), "LEVEL=ERROR")
}

func TestLogVertexStructuredOutputExhaustedKeepsInvalidRequestsAsErrors(t *testing.T) {
	var logBuffer bytes.Buffer
	previous := slog.Default()
	slog.SetDefault(slog.New(slog.NewTextHandler(&logBuffer, &slog.HandlerOptions{Level: slog.LevelDebug})))
	t.Cleanup(func() {
	...
	logVertexStructuredOutputExhausted(
		...
		1,
		time.Second,
		errors.New("400 invalid payload"),
		"vertex_ai",
	)

	logText := logBuffer.String()
	assert.Contains(t, logText, "level=ERROR")
	assert.Contains(t, logText, "error_class=invalid_request")
}
...
func TestIsProviderRateLimitError(t *testing.T) {
	...
	assert.False(t, IsProviderRateLimitError(errors.New("mock unexpected call at 0x7ff7df514290")))
}

func TestClassifyVertexProviderCancellation(t *testing.T) {
	assert.Equal(t, "canceled", classifyVertexError(errors.New("Error 499, Message: The operation was cancelled., Status: CANCELLED")))
...
func TestVertexClient_GenerateStructuredWithTokens_DisablesDynamicThinkingForTinyOutputBudget(t *testing.T) {
	mm := new(mockGenAIModels)
	vc := &VertexClient{client: mm, backend: "vertex_ai"}
	ctx := context.Background()
	dynamicBudget := int32(-1)
	...
	text, _, _, err := vc.generateStructuredWithTokens(ctx, "gemini-2.5-flash-lite", "prompt", "", `{"type":"object"}`, 0.3, 64, "", &dynamicBudget, "structured_high_value", "strict_json")
	assert.NoError(t, err)
	assert.Equal(t, `{"ok":true}`, text)
	mm.AssertExpectations(t)
}
...
func TestVertexClient_EmbedText_NoEmbeddingReturned(t *testing.T) {
	...
	_, err := vc.EmbedText(ctx, "", "query text", "query")
	assert.Error(t, err)
