...
func NewResourceGovernorWithInterval(maxMemMB uint64, maxRoutines int, interval time.Duration) *ResourceGovernor {
	return NewResourceGovernorWithLimits(maxMemMB, maxRoutines, maxRoutines, interval)
}

// NewResourceGovernorWithLimits creates a governor with separate runtime
// health thresholds and active task admission limits.
func NewResourceGovernorWithLimits(maxMemMB uint64, maxRoutines int, maxActiveTasks int, interval time.Duration) *ResourceGovernor {
	g := &ResourceGovernor{
