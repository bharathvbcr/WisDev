...
type SearchResult struct {
	...
	RawResultCount   int               `json:"rawResultCount,omitempty"`
	FusedResultCount int               `json:"fusedResultCount,omitempty"`
	DedupedCount     int               `json:"dedupedCount,omitempty"`
...
func stableSearchQueryHash(query string) string {
...
func buildProviderSearchLogArgs(stage string, provider string, query string, opts SearchOpts, attrs ...any) []any {
	...
	queryHash := ""
	if normalizedQuery != "" {
		queryHash = stableSearchQueryHash(normalizedQuery)
	}
	base := []any{
	...
	if traceID := strings.TrimSpace(opts.TraceID); traceID != "" {
		base = append(base, "trace_id", traceID)
	}
	base = append(base,
...
func buildProviderSearchLogArgsForContext(ctx context.Context, stage string, provider string, query string, opts SearchOpts, attrs ...any) []any {
	if strings.TrimSpace(opts.TraceID) == "" {
		opts.TraceID = telemetry.TraceIDFrom(ctx)
	}
	return buildProviderSearchLogArgs(stage, provider, query, opts, attrs...)
}

func logProviderSearchStage(ctx context.Context, stage string, provider string, query string, opts SearchOpts, attrs ...any) {
	telemetry.Logger().InfoContext(ctx, "academic provider search lifecycle", buildProviderSearchLogArgsForContext(ctx, stage, provider, query, opts, attrs...)...)
}

func logProviderSearchFailure(ctx context.Context, stage string, provider string, query string, opts SearchOpts, attrs ...any) {
	telemetry.Logger().WarnContext(ctx, "academic provider search lifecycle", buildProviderSearchLogArgsForContext(ctx, stage, provider, query, opts, attrs...)...)
}
...
func (r *ProviderRegistry) AdjustConcurrency(name string, err error) {
	...
	minCap := defaultProviderConcurrency(name)
	maxCap := maxProviderConcurrency(name)
...
func (r *ProviderRegistry) waitForProviderRateGate(ctx context.Context, name string) (time.Duration, error) {
	...
	for {
		...
		now := time.Now()
		wait := r.providerLastRequest[providerName].Add(interval).Sub(now)
		if wait <= 0 {
			r.providerLastRequest[providerName] = now
			r.mu.Unlock()
			return 0, nil
		}
		r.mu.Unlock()
	...
	go func() {
		...
		for _, p := range providers {
			...
			go func(prov SearchProvider) {
				...
				if rateGateErr != nil {
					select {
					case out <- ProviderResult{Provider: prov.Name(), Err: fmt.Errorf("provider rate gate canceled: %w", rateGateErr)}:
					case <-ctx.Done():
					}
					return
				}
				if rateGateWait > 0 {
					slog.Info("academic provider rate gate waited",
						"component", "search.parallel",
						"operation", "provider_search",
						"stage", "rate_gate_waited",
						"provider", prov.Name(),
...
func ParallelSearch(ctx context.Context, reg *ProviderRegistry, query string, opts SearchOpts) SearchResult {
		...
		go func(prov SearchProvider, cb *resilience.CircuitBreaker, s *semaphore.Weighted) {
			...
			if rateGateErr != nil {
				...
				logProviderSearchFailure(pctx, "rate_gate_cancelled", prov.Name(), query, opts,
					"result", "failure",
					"error_code", "PROVIDER_RATE_GATE_CANCELLED",
					"error", rateGateErr.Error(),
				)
				results <- ProviderResult{
					Provider: prov.Name(),
					Err:      fmt.Errorf("provider rate gate canceled: %w", rateGateErr),
				}
				return
			}
			if rateGateWait > 0 {
				pspan.SetAttributes(attribute.Int64("provider_rate_gate_wait_ms", rateGateWait.Milliseconds()))
				logProviderSearchStage(pctx, "rate_gate_waited", prov.Name(), query, opts,
					"latency_ms", rateGateWait.Milliseconds(),
					"result", "waited",
				)
			}
			...
			logProviderSearchStage(pctx, "provider_dispatch_start", prov.Name(), query, opts,
				"result", "started",
			)
			if cb != nil {
	...
	slog.Info("academic search provider fan-out complete",
		"service", "go_orchestrator",
		"runtime", "go",
		"component", "search.parallel",
		"operation", "parallel_search",
		"stage", "provider_fanout_complete",
		"query_hash", stableSearchQueryHash(query),
		"query_preview", queryPreview(query),
		"domain", opts.Domain,
		"provider_count", len(providers),
		"provider_result_counts", providerCounts,
	...
	slog.Info("academic search result finalized",
		"service", "go_orchestrator",
		"runtime", "go",
		"component", "search.parallel",
		"operation", "parallel_search",
		...
		"query_preview", queryPreview(query),
		"domain", opts.Domain,
		"raw_result_count", finalResult.RawResultCount,
		"fused_result_count", finalResult.FusedResultCount,
		"deduped_count", finalResult.DedupedCount,
		"final_count", finalResult.FinalCount,
		"limit", limit,
		"limit_applied", finalResult.LimitApplied,
		"latency_ms", finalResult.LatencyMs,
		"warning_count", len(finalResult.Warnings),
	)
