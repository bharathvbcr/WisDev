...
func TestBuildHITLRequestUsesConfiguredActions(t *testing.T) {
	...
	if request["adkConfirmationFunction"] != toolconfirmation.FunctionCallName {
		t.Fatalf("expected official ADK confirmation function, got %#v", request["adkConfirmationFunction"])
	}
	confirmation, ok := request["adkRequestedToolConfirmation"].(toolconfirmation.ToolConfirmation)
	if !ok {
		t.Fatalf("expected ToolConfirmation, got %T", request["adkRequestedToolConfirmation"])
	}
	if confirmation.Hint != "review replanning step" {
		t.Fatalf("unexpected ADK confirmation hint %q", confirmation.Hint)
	}
}
...
func TestMetadataReportsOfficialADKDiagnostics(t *testing.T) {
	runtime := &ADKRuntime{
		Config:           DefaultADKRuntimeConfig(),
		CredentialSource: "vertex_ai:env:GOOGLE_CLOUD_PROJECT",
		ModelBackend:     "vertex_ai",
	}

	meta := runtime.Metadata()
	if meta["officialModule"] != officialADKModule {
		t.Fatalf("expected official ADK module marker, got %#v", meta["officialModule"])
	}
	if meta["framework"] != "google-adk-go" {
		t.Fatalf("expected configured ADK framework, got %#v", meta["framework"])
	}
	if meta["credentialSource"] != "vertex_ai:env:GOOGLE_CLOUD_PROJECT" {
		t.Fatalf("expected non-secret credential source, got %#v", meta["credentialSource"])
	}
	if meta["modelBackend"] != "vertex_ai" {
		t.Fatalf("expected model backend, got %#v", meta["modelBackend"])
	}
	if _, ok := meta["moduleVersion"].(string); !ok {
		t.Fatalf("expected module version field, got %#v", meta["moduleVersion"])
	}
}
