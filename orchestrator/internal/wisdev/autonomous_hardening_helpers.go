...
func (l *AutonomousLoop) critiqueDraft(ctx context.Context, query string, draft string, papers []search.Paper, evidenceItems []EvidenceItem, gap *LoopGapState) *LoopDraftCritique {
	...
	if remaining := autonomousLLMCooldownRemaining(l); remaining > 0 {
		if shouldLogWisDevCooldownFallback("wisdev.autonomous.critiqueDraft", time.Now()) {
...
func mergePostRetrievalDraftCritique(original *LoopDraftCritique, review *LoopDraftCritique, retrievalReopened bool, additionalEvidenceFound bool) *LoopDraftCritique {
	if review == nil {
		return original
	}
	review.RetrievalReopened = retrievalReopened
	review.AdditionalEvidenceFound = additionalEvidenceFound
	if original == nil {
		return review
	}
	originalReasoning := strings.TrimSpace(original.Reasoning)
	reviewReasoning := strings.TrimSpace(review.Reasoning)
	if originalReasoning != "" && reviewReasoning != "" && !strings.Contains(strings.ToLower(reviewReasoning), strings.ToLower(originalReasoning)) {
		review.Reasoning = originalReasoning + " Post-retrieval review: " + reviewReasoning
	}
	if review.NeedsRevision {
		review.NextQueries = dedupeTrimmedStrings(append(review.NextQueries, original.NextQueries...))
		review.MissingAspects = dedupeTrimmedStrings(append(review.MissingAspects, original.MissingAspects...))
		review.MissingSourceTypes = dedupeTrimmedStrings(append(review.MissingSourceTypes, original.MissingSourceTypes...))
		review.Contradictions = dedupeTrimmedStrings(append(review.Contradictions, original.Contradictions...))
	}
	return review
}

// BuildLoopAutonomousReviewMetadata summarizes autonomous critique and reasoning state for transport payloads.
func BuildLoopAutonomousReviewMetadata(loopResult *LoopResult) map[string]any {
	return buildLoopAutonomousReviewMetadata(loopResult)
}
...
func buildLoopAutonomousReviewMetadata(loopResult *LoopResult) map[string]any {
	...
	if critique := loopResult.DraftCritique; critique != nil {
		status := "passed"
		switch {
		case critique.NeedsRevision:
			status = "revision_required"
		case critique.RetrievalReopened && critique.AdditionalEvidenceFound:
			status = "resolved_after_follow_up"
		case critique.RetrievalReopened:
			status = "retrieval_reopened"
		}
		review["status"] = status
		review["needsRevision"] = critique.NeedsRevision
		review["retrievalReopened"] = critique.RetrievalReopened
		review["additionalEvidenceFound"] = critique.AdditionalEvidenceFound
		review["reasoning"] = strings.TrimSpace(critique.Reasoning)
		review["confidence"] = critique.Confidence
		review["nextQueries"] = stringSliceToAny(critique.NextQueries)
	}
	if len(loopResult.ReasoningTrace) > 0 {
		review["reasoningTrace"] = valueToAny(loopResult.ReasoningTrace)
		review["traceStepCount"] = len(loopResult.ReasoningTrace)
		review["postCritiqueReview"] = reasoningTraceContainsDecision(loopResult.ReasoningTrace, "post_critique_review")
	}
	if loopResult.GapAnalysis != nil {
		review["openLedgerCount"] = countOpenCoverageLedgerEntries(loopResult.GapAnalysis.Ledger)
		review["gapSufficient"] = loopResult.GapAnalysis.Sufficient
	}
	if stopReason := strings.TrimSpace(loopResult.StopReason); stopReason != "" {
		review["stopReason"] = stopReason
	}
	if len(review) == 0 {
		return nil
	}
...
func reasoningTraceContainsDecision(trace []ReasoningTraceEntry, decision string) bool {
	decision = strings.ToLower(strings.TrimSpace(decision))
	if decision == "" {
		return false
	}
	for _, entry := range trace {
		if strings.ToLower(strings.TrimSpace(entry.Decision)) == decision {
			return true
		}
	}
	return false
...
func mergeDraftCritiqueIntoGapState(gap *LoopGapState, critique *LoopDraftCritique, query string) *LoopGapState {
	...
	case !critique.NeedsRevision && critique.RetrievalReopened && critique.AdditionalEvidenceFound:
		title = "Draft critique reopened retrieval and final review passed"
