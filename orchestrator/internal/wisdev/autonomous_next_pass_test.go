...
func TestAutonomousLoopHelpers_QueryManagement(t *testing.T) {
	...
	t.Run("record and clone loop coverage", func(t *testing.T) {
		coverage := map[string][]search.Paper{
			"  primary ": {
				{ID: "openalex:p1", Title: "Primary"},
			},
			"invalid":    {{ID: "skip"}},
			"secondary ": nil,
		}
		recordLoopQueryCoverage(coverage, "", []search.Paper{{ID: "openalex:p2"}})
		recordLoopQueryCoverage(coverage, "primary", []search.Paper{{ID: "openalex:p2"}})
		recordLoopQueryCoverage(nil, "none", []search.Paper{{ID: "p3"}})

		clone := cloneLoopQueryCoverage(coverage)
		assert.Len(t, clone["primary"], 2)
		assert.Contains(t, []string{"openalex:p1", "openalex:p2"}, clone["primary"][0].ID)
...
func TestAutonomousLoopHelpers_Resolution(t *testing.T) {
	t.Run("resolvers cover defaults", func(t *testing.T) {
		...
		assert.True(t, isHighDepthResearchPlane(ResearchExecutionPlaneJob))
	...
	t.Run("adaptive parallelism scales with confidence", func(t *testing.T) {
		...
		assert.Equal(t, 4, loop.resolveAdaptiveParallelism(string(WisDevModeYOLO), 0.2, ResearchExecutionPlaneSimple))
		assert.Equal(t, 3, loop.resolveAdaptiveParallelism(string(WisDevModeYOLO), 0.5, ResearchExecutionPlaneAutonomous))
	})

	t.Run("adaptive loop recursion and critique limits", func(t *testing.T) {
		assert.Equal(t, 3, resolveLoopGapRecursionDepth(string(WisDevModeYOLO), ResearchExecutionPlaneAutonomous))
		assert.Equal(t, 3, resolveLoopGapRecursionDepth(string(WisDevModeYOLO), ResearchExecutionPlaneJob))
		assert.Equal(t, 2, resolveLoopGapRecursionDepth(string(WisDevModeYOLO), ResearchExecutionPlaneSimple))
		...
		assert.Equal(t, 4, resolveCritiqueFollowUpLimit(string(WisDevModeYOLO), ResearchExecutionPlaneAutonomous))
		assert.Equal(t, 4, resolveCritiqueFollowUpLimit(string(WisDevModeYOLO), ResearchExecutionPlaneJob))
		assert.Equal(t, 3, resolveCritiqueFollowUpLimit(string(WisDevModeYOLO), ResearchExecutionPlaneSimple))
	...
	t.Run("guided feedback waits for planned agenda coverage", func(t *testing.T) {
		...
		partialExecuted := []string{
			"sleep and memory",
			"hippocampal replay",
			"systems consolidation",
		}
		completeExecuted := []string{
			"sleep and memory",
			"hippocampal replay",
			"systems consolidation",
			"slow-wave sleep",
		...
		guidedReq := LoopRequest{Mode: string(WisDevModeGuided), ResearchPlane: ResearchExecutionPlaneAutonomous}
		assert.False(t, shouldEnableBeliefFeedback(guidedReq, initialAgenda, partialExecuted))
		assert.False(t, shouldBootstrapLoopHypotheses(
			guidedReq,
			nil,
			[]EvidenceFinding{{ID: "finding-1", Claim: "seed"}},
			initialAgenda,
			partialExecuted,
		))

		assert.True(t, shouldEnableBeliefFeedback(guidedReq, initialAgenda, completeExecuted))
		assert.True(t, shouldBootstrapLoopHypotheses(
			guidedReq,
			nil,
			[]EvidenceFinding{{ID: "finding-1", Claim: "seed"}},
			initialAgenda,
		...
		yoloReq := LoopRequest{Mode: string(WisDevModeYOLO), ResearchPlane: ResearchExecutionPlaneAutonomous}
		assert.True(t, shouldEnableBeliefFeedback(yoloReq, initialAgenda, partialExecuted))
	})
	...
	t.Run("dynamic provider selection honors cooldown and plane", func(t *testing.T) {
		...
		assert.False(t, shouldUseDynamicProviderSelection(string(WisDevModeYOLO), ResearchExecutionPlaneSimple, true, &llm.Client{VertexDirect: &llm.VertexClient{}}))
		assert.True(t, shouldUseDynamicProviderSelection(string(WisDevModeYOLO), ResearchExecutionPlaneDeep, true, &llm.Client{VertexDirect: &llm.VertexClient{}}))
	})
	...
	t.Run("emitLoopProgress handles nil emitters and normalizes payload", func(t *testing.T) {
		...
		assert.Equal(t, "orchestrator/internal/wisdev", seen[0].OwningComponent)
	...
	t.Run("autonomous loop start event carries mode observability", func(t *testing.T) {
		reg := search.NewProviderRegistry()
		seenTraceIDs := make([]string, 0, 1)
		reg.Register(&mockSearchProvider{
			...
			SearchFunc: func(ctx context.Context, query string, opts search.SearchOpts) ([]search.Paper, error) {
				seenTraceIDs = append(seenTraceIDs, opts.TraceID)
				return []search.Paper{{ID: "p1", Title: "paper"}}, nil
			},
		})
		...
		_, err := loop.Run(
			...
			LoopRequest{
				Query:           "rlhf",
				ProjectID:       "session-42",
				TraceID:         "trace-42",
				Mode:            string(WisDevModeYOLO),
				ResearchPlane:   ResearchExecutionPlaneAutonomous,
				MaxIterations:   1,
				MaxSearchTerms:  1,
				HitsPerSearch:   2,
				MaxUniquePapers: 3,
		...
		assert.Equal(t, "trace-42", seen[0].TraceID)
		assert.Equal(t, "yolo", seen[0].Payload["executionMode"])
		assert.Equal(t, string(ResearchExecutionPlaneAutonomous), seen[0].Payload["researchPlane"])
		assert.Equal(t, 1, seen[0].Payload["maxSearchTerms"])
		assert.Equal(t, 2, seen[0].Payload["hitsPerSearch"])
		assert.Contains(t, seenTraceIDs, "trace-42")
	})
}
...
func TestAutonomousLoopHelpers_ExecutionAndLimits(t *testing.T) {
	...
	t.Run("executeLoopSearchBatch includes trace id in batch lifecycle logs", func(t *testing.T) {
		...
		reg.Register(&mockSearchProvider{
			name:   "mock",
			papers: []search.Paper{{ID: "p1", Title: "Paper"}},
		})
		loop := NewAutonomousLoop(reg, nil)
		...
		results := loop.executeLoopSearchBatch(
			context.Background(),
			[]string{"traceable"},
			search.SearchOpts{Limit: 1, TraceID: "trace-batch-lifecycle"},
			1,
		)

		assert.Len(t, results, 1)
		logs := output.String()
		assert.Contains(t, logs, `"msg":"wisdev search batch started"`)
		assert.Contains(t, logs, `"stage":"batch_started"`)
		assert.Contains(t, logs, `"msg":"wisdev search query completed"`)
		assert.Contains(t, logs, `"stage":"query_completed"`)
		assert.Contains(t, logs, `"trace_id":"trace-batch-lifecycle"`)
	})
	...
	t.Run("closeRecursiveCoverageGaps dedupes candidates against seen queries", func(t *testing.T) {
		...
		reg.Register(&mockSearchProvider{
			...
			SearchFunc: func(ctx context.Context, query string, opts search.SearchOpts) ([]search.Paper, error) {
				...
				assert.Equal(t, "trace-recursive-gap", opts.TraceID)
				return []search.Paper{{ID: "openalex:unique-paper", Title: "Unique Paper"}}, nil
			},
		})
		loop := NewAutonomousLoop(reg, nil)
		result, err := loop.closeRecursiveCoverageGaps(
			...
			LoopRequest{
				...
				TraceID:       "trace-recursive-gap",
			},
			[]string{"seed"},
			[]string{"seed"},
			map[string][]search.Paper{"seed": {{ID: "openalex:p1", Title: "Seed Paper", FullText: "available"}}},
			[]search.Paper{{ID: "openalex:p1", Title: "Seed Paper", FullText: "available"}},
