...
func TestAutonomousLoop_ParsesFencedJSONAndDegradesQuotaErrors(t *testing.T) {
	...
	t.Run("heuristic sufficiency does not prematurely declare completion", func(t *testing.T) {
		analysis := heuristicsufficiencyAnalysisWithoutLLM("test query", []search.Paper{{
			ID:       "p1",
			Title:    "Heuristic Coverage",
			Abstract: "Grounded evidence describes test query replication evidence.",
			Source:   "openalex",
		...
		assert.Contains(t, analysis.Reasoning, "without declaring the research complete")
		assert.NotEmpty(t, analysis.MissingAspects)
	})
...
func TestAutonomousLoopCarriesInitialMemoryTiers(t *testing.T) {
	reg := search.NewProviderRegistry()
	reg.Register(&mockSearchProvider{
		name: "memory_context",
		SearchFunc: func(ctx context.Context, query string, opts search.SearchOpts) ([]search.Paper, error) {
			return []search.Paper{{
				ID:       stableWisDevID("memory-paper", query),
				Title:    query + " evidence",
				Abstract: "Fresh evidence for " + query,
				Source:   "semantic_scholar",
			}}, nil
		},
	})
	reg.SetDefaultOrder([]string{"memory_context"})

	res, err := NewAutonomousLoop(reg, nil).Run(context.Background(), LoopRequest{
		Query:           "room-temperature superconductors replication pitfalls",
		...
		HitsPerSearch:   1,
		MaxUniquePapers: 3,
		InitialMemoryTiers: &MemoryTierState{
			LongTermVector: []MemoryEntry{{
				ID:              "prior-vector",
				Type:            "evidence",
				Content:         "Historical LK-99 replication failures often involved measurement artifacts.",
				CreatedAt:       42,
				EvaluationScore: 0.9,
			}},
			ArtifactMemory: []MemoryEntry{{
				ID:        "prior-artifact",
				Type:      "episode",
				Content:   "Prior session flagged magnetic impurity controls.",
				CreatedAt: 43,
			}},
		},
	})
	assert.NoError(t, err)
	if assert.NotNil(t, res) {
		assert.Contains(t, reasoningTraceDecisions(res.ReasoningTrace), "memory_context_refresh")
	}
	if assert.NotNil(t, res) && assert.NotNil(t, res.MemoryTiers) {
		assert.Contains(t, memoryEntryIDs(res.MemoryTiers.LongTermVector), "prior-vector")
		assert.Contains(t, memoryEntryIDs(res.MemoryTiers.ArtifactMemory), "prior-artifact")
		assert.Greater(t, len(res.MemoryTiers.LongTermVector), 1)
	}
}

func reasoningTraceDecisions(trace []ReasoningTraceEntry) []string {
	out := make([]string, 0, len(trace))
	for _, entry := range trace {
		out = append(out, entry.Decision)
	}
	return out
}

func memoryEntryIDs(entries []MemoryEntry) []string {
