...
func (c *BrainCapabilities) ResolveCanonicalCitations(ctx context.Context, papers []Source, model string) (map[string]any, error) {
	...
	resp, err := c.llmClient.StructuredOutput(requestCtx, applyWisdevRecoverableStructuredPolicy(&llmv1.StructuredRequest{
		Prompt:     prompt,
		Model:      model,
		JsonSchema: `{"type": "object", "properties": {"canonicalSources": {"type": "array", "items": {"type": "object", "properties": {"id": {"type": "string"}, "title": {"type": "string"}, "doi": {"type": "string"}, "arxivId": {"type": "string"}, "canonicalId": {"type": "string"}, "resolved": {"type": "boolean"}}}}, "resolved": {"type": "array", "items": {"type": "object", "properties": {"id": {"type": "string"}, "canonicalId": {"type": "string"}}}}, "resolvedCount": {"type": "integer"}, "unresolvedCount": {"type": "integer"}}}`,
	}))
	if err != nil {
		if llm.IsProviderRateLimitError(err) {
...
func fallbackCanonicalCitations(papers []Source) map[string]any {
	...
	canonicalSources := make([]map[string]any, 0, len(papers))
	for i, paper := range papers {
		...
		canonicalSources = append(canonicalSources, canonicalCitationRecordFromSource(paper, i, map[string]any{
			"id":          id,
			"canonicalId": canonicalID,
			"degraded":    true,
			"resolved":    true,
	...
	return map[string]any{
		"resolved":         resolved,
		"canonicalSources": mapsToAny(canonicalSources),
		"citations":        mapsToAny(canonicalSources),
		"resolvedCount":    len(canonicalSources),
		"unresolvedCount":  0,
		"degraded":         true,
	}
}

func normalizeCanonicalCitationResult(result map[string]any, papers []Source) map[string]any {
	result = ensureExecutionResultMap(result)
	canonical := firstArtifactMaps(firstNonEmptyValue(result["canonicalSources"], result["citations"]))
	if len(canonical) == 0 {
		canonical = firstArtifactMaps(result["resolved"])
	}
	if len(canonical) == 0 && len(papers) > 0 {
		canonical = make([]map[string]any, 0, len(papers))
		for i, paper := range papers {
			canonical = append(canonical, canonicalCitationRecordFromSource(paper, i, nil))
		}
	} else {
		for i, record := range canonical {
			canonical[i] = canonicalCitationRecordFromSource(matchCitationSource(record, papers), i, record)
		}
	}
	if len(canonical) > 0 {
		asAny := mapsToAny(canonical)
		result["canonicalSources"] = asAny
		result["citations"] = asAny
		if _, ok := result["resolvedCount"]; !ok {
			result["resolvedCount"] = len(canonical)
		}
		if _, ok := result["unresolvedCount"]; !ok {
...
func matchCitationSource(record map[string]any, papers []Source) Source {
	id := AsOptionalString(firstNonEmptyValue(record["id"], record["paperId"], record["doi"], record["arxivId"], record["arxiv_id"], record["title"]))
	canonicalID := AsOptionalString(firstNonEmptyValue(record["canonicalId"], record["canonical_id"]))
	for _, paper := range papers {
		keys := []string{paper.ID, paper.DOI, paper.ArxivID, paper.Title}
		for _, key := range keys {
			if strings.EqualFold(strings.TrimSpace(key), strings.TrimSpace(id)) || strings.EqualFold(strings.TrimSpace(key), strings.TrimSpace(canonicalID)) {
				return paper
			}
		}
	}
...
func canonicalCitationRecordFromSource(paper Source, index int, record map[string]any) map[string]any {
	out := cloneAnyMap(record)
	id := firstNonEmpty(
		AsOptionalString(firstNonEmptyValue(out["id"], out["paperId"])),
		paper.ID,
		paper.DOI,
		paper.ArxivID,
		paper.Title,
		fmt.Sprintf("source_%d", index+1),
	)
	title := firstNonEmpty(AsOptionalString(out["title"]), paper.Title, id)
	doi := firstNonEmpty(AsOptionalString(out["doi"]), paper.DOI)
	arxivID := firstNonEmpty(AsOptionalString(firstNonEmptyValue(out["arxivId"], out["arxiv_id"])), paper.ArxivID)
	canonicalID := firstNonEmpty(
		AsOptionalString(firstNonEmptyValue(out["canonicalId"], out["canonical_id"])),
		doi,
		arxivID,
		paper.ID,
		stableWisDevID("citation", title, paper.Link),
	)
	out["id"] = id
	out["title"] = title
