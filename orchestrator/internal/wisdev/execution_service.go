...
const executionEventIdleLogInterval = 15 * time.Second
...
func (s *DurableExecutionService) Start(ctx context.Context, sessionID string) (*ExecutionStartResult, error) {
	...
	if err := prepareSessionForExecution(ctx, s.gateway, session); err != nil {
		return nil, err
	}
...
func syntheticTerminalExecutionEvent(session *AgentSession) PlanExecutionEvent {
	...
	return PlanExecutionEvent{
		...
		Message:   message,
		CreatedAt: NowMillis(),
		Payload:   buildExecutionTerminalPayload(session, payloadStatus, true),
	}
}
...
func executeSessionRun(ctx context.Context, gateway *AgentGateway, sessionID string) (*AgentSession, error) {
	...
	if err := prepareSessionForExecution(ctx, gateway, session); err != nil {
		return nil, err
	}
	...
	runnerMode := "plan_executor"
	if gateway.ADKRuntime != nil && gateway.ADKRuntime.Runner != nil && gateway.ADKRuntime.Agent != nil {
		runnerMode = "adk_runtime"
		logExecutionRunnerSelected(session, runnerMode, cap(events))
		go func() {
			defer close(events)
			if err := gateway.ADKRuntime.ExecuteSession(ctx, session, func(event PlanExecutionEvent) {
				select {
				case events <- event:
				case <-ctx.Done():
				}
			}); err != nil {
				slog.Error("wisdev ADK session runner failed",
					"service", "go_orchestrator",
					"runtime", "go",
					"component", "wisdev.execution_service",
					"operation", "execute_session_run",
					"stage", "adk_runner_failed",
					"session_id", session.SessionID,
					"plan_id", session.PlanID(),
					"error", err.Error(),
				)
				failureEvent := PlanExecutionEvent{
	...
	idleTicker := time.NewTicker(executionEventIdleLogInterval)
	defer idleTicker.Stop()
	for {
		select {
		case <-ctx.Done():
			slog.Warn("wisdev execution event wait cancelled",
				"service", "go_orchestrator",
				"runtime", "go",
				"component", "wisdev.execution_service",
				"operation", "execute_session_run",
				"stage", "event_wait_cancelled",
				"session_id", session.SessionID,
				"plan_id", session.PlanID(),
				"runner_mode", runnerMode,
				"event_count", eventCount,
				"idle_ms", time.Since(lastEventAt).Milliseconds(),
				"error", ctx.Err().Error(),
			)
			return session, ctx.Err()
		case <-idleTicker.C:
			logExecutionEventWaitHeartbeat(session, runnerMode, eventCount, lastEventAt)
			continue
		case event, ok := <-events:
			if !ok {
				slog.Info("wisdev execution event channel closed",
					"service", "go_orchestrator",
					"runtime", "go",
					"component", "wisdev.execution_service",
					"operation", "execute_session_run",
					"stage", "event_channel_closed",
					"session_id", session.SessionID,
					"plan_id", session.PlanID(),
					"runner_mode", runnerMode,
					"event_count", eventCount,
			...
			eventCount++
			lastEventAt = time.Now()
			logExecutionEventReceived(session, event, runnerMode, eventCount)
			applyExecutionEventStatus(session, event)
...
func prepareSessionForExecution(ctx context.Context, gateway *AgentGateway, session *AgentSession) error {
	if gateway == nil || gateway.Store == nil || session == nil {
		return fmt.Errorf("execution service unavailable")
	}
	if session.Status != SessionQuestioning {
		return nil
	}
	if session.Plan == nil {
		return fmt.Errorf("session execution plan is required")
	}
	...
	slog.Info("wisdev execution lifecycle prepared",
		"service", "go_orchestrator",
		"runtime", "go",
		"component", "wisdev.execution_service",
		"operation", "prepare_session_for_execution",
		"stage", "status_advanced",
		"session_id", session.SessionID,
		"plan_id", session.PlanID(),
		"previous_status", string(previous),
		"next_status", string(session.Status),
		"reason", "planned_session_execution",
	)
	return nil
}

func applyExecutionEventStatus(session *AgentSession, event PlanExecutionEvent) {
	if session == nil {
		return
	}
	applyExecutionEventPlanProgress(session, event)
	switch event.Type {
	case EventCompleted:
		applyExecutionTerminalPayloadProgress(session, event.Payload)
...
func applyExecutionEventPlanProgress(session *AgentSession, event PlanExecutionEvent) {
	if session == nil || session.Plan == nil || strings.TrimSpace(event.StepID) == "" {
		return
	}
	...
	attempts := intFromAny(event.Payload["attempts"])
	if attempts == 0 {
		attempts = intFromAny(event.Payload["attempt"])
	}
	if attempts > session.Plan.StepAttempts[event.StepID] {
		session.Plan.StepAttempts[event.StepID] = attempts
	}
	if event.ResultConfidence > 0 {
		session.Plan.StepConfidences[event.StepID] = event.ResultConfidence
	}

	switch event.Type {
	case EventStepCompleted:
		session.Plan.CompletedStepIDs[event.StepID] = true
		delete(session.Plan.FailedStepIDs, event.StepID)
	case EventStepFailed:
		if strings.TrimSpace(event.Message) != "" {
			session.Plan.FailedStepIDs[event.StepID] = event.Message
		} else {
			session.Plan.FailedStepIDs[event.StepID] = "step failed"
		}
		session.Plan.StepFailureCount[event.StepID]++
		delete(session.Plan.CompletedStepIDs, event.StepID)
	}
}

func logExecutionRunnerSelected(session *AgentSession, runnerMode string, channelCapacity int) {
	if session == nil {
		return
	}
	slog.Info("wisdev execution runner selected",
		"service", "go_orchestrator",
		"runtime", "go",
		"component", "wisdev.execution_service",
		"operation", "execute_session_run",
		"stage", "runner_selected",
		"session_id", session.SessionID,
		"plan_id", session.PlanID(),
		"runner_mode", runnerMode,
...
func logExecutionEventReceived(session *AgentSession, event PlanExecutionEvent, runnerMode string, eventCount int) {
	if session == nil {
		return
	}
	slog.Info("wisdev execution event received",
		"service", "go_orchestrator",
		"runtime", "go",
		"component", "wisdev.execution_service",
		"operation", "execute_session_run",
		...
		"result_count", intFromAny(event.Payload["resultCount"]),
	)
}

func logExecutionEventWaitHeartbeat(session *AgentSession, runnerMode string, eventCount int, lastEventAt time.Time) {
	if session == nil {
		return
	}
	slog.Info("wisdev execution waiting for runner events",
		"service", "go_orchestrator",
		"runtime", "go",
		"component", "wisdev.execution_service",
		"operation", "execute_session_run",
		"stage", "event_wait_heartbeat",
		"session_id", session.SessionID,
		"plan_id", session.PlanID(),
		"runner_mode", runnerMode,
		"event_count", eventCount,
