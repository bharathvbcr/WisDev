...
func TestExecuteSessionRunUsesOfficialADKRunnerWhenAvailable(t *testing.T) {
	t.Setenv("WISDEV_JOURNAL_PATH", filepath.Join(t.TempDir(), "wisdev_journal.jsonl"))
	gateway := NewAgentGateway(nil, nil, nil)
	gateway.Journal = NewRuntimeJournal(nil)
		...
		Run: func(ctx agent.InvocationContext) iter.Seq2[*adksession.Event, error] {
			return func(yield func(*adksession.Event, error) bool) {
				...
				event.CustomMetadata = map[string]any{
					...
					"sessionId":    "adk-execution-session",
					"planId":       "plan-adk",
					"message":      "ADK completed",
					"resultOrigin": "adk_session_runner",
				}
	...
	finalSession, err := executeSessionRun(context.Background(), gateway, session.SessionID)

	require.NoError(t, err)
	require.NotNil(t, finalSession)
	assert.Equal(t, 0, direct.calls)
	assert.Equal(t, SessionComplete, finalSession.Status)
	entries := gateway.Journal.ReadSession(session.SessionID, 16)
