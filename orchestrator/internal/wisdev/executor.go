...
func defaultGoNativeStepLimit(action string) int {
	switch CanonicalizeWisdevAction(action) {
	case ActionResearchRetrievePapers, "search":
		return defaultRetrievePapersLimit
	default:
		return defaultGoNativeLimit
	}
}
...
func buildStepHandoffLogArgs(stage string, session *AgentSession, step PlanStep, ownership stepExecutionOwnership, degraded bool, attrs ...any) []any {
	...
	if session != nil {
		...
		if session.Plan != nil {
			planID = strings.TrimSpace(session.Plan.PlanID)
		}
	}
	base := []any{
		...
		"plan_id", planID,
		"step_id", strings.TrimSpace(step.ID),
		"action", strings.TrimSpace(step.Action),
		"execution_target", string(step.ExecutionTarget),
		"query_preview", QueryPreview(query),
		"query_length", len(strings.TrimSpace(query)),
		"degraded", degraded,
		"delegated", strings.TrimSpace(ownership.ResultOrigin) != "" && ownership.ResultOrigin != executionOrigin,
		"owner", strings.TrimSpace(ownership.Owner),
		"sub_agent", strings.TrimSpace(ownership.SubAgent),
		"owning_component", strings.TrimSpace(ownership.OwningComponent),
		"result_origin", strings.TrimSpace(ownership.ResultOrigin),
		"fusion_intent", strings.TrimSpace(ownership.ResultFusionIntent),
	}
	droppedMalformedAttrs := 0
	for idx := 0; idx < len(attrs); idx += 2 {
		key, ok := attrs[idx].(string)
		key = strings.TrimSpace(key)
		if !ok || key == "" {
			droppedMalformedAttrs++
			continue
		}
		if idx+1 >= len(attrs) {
			droppedMalformedAttrs++
		...
		base = append(base, key, attrs[idx+1])
	}
	if droppedMalformedAttrs > 0 {
		base = append(base, "dropped_malformed_attr_count", droppedMalformedAttrs)
	}
	return base
}

func logStepHandoffStage(ctx context.Context, stage string, session *AgentSession, step PlanStep, ownership stepExecutionOwnership, degraded bool, attrs ...any) {
	ctx = executorContext(ctx)
	telemetry.FromCtx(ctx).InfoContext(ctx, "wisdev agent query handoff", buildStepHandoffLogArgs(stage, session, step, ownership, degraded, attrs...)...)
}

func logStepHandoffFailure(ctx context.Context, stage string, session *AgentSession, step PlanStep, ownership stepExecutionOwnership, degraded bool, attrs ...any) {
	ctx = executorContext(ctx)
	telemetry.FromCtx(ctx).WarnContext(ctx, "wisdev agent query handoff", buildStepHandoffLogArgs(stage, session, step, ownership, degraded, attrs...)...)
}
...
func (e *PlanExecutor) executeDelegatedStep(
	...
	case ExecutionTargetGoNative:
		if _, ok := payload["limit"]; !ok {
			payload["limit"] = defaultGoNativeStepLimit(step.Action)
		}
		payload["maxAttempts"] = step.MaxAttempts
		if degraded {
			payload["query"] = degradedQuery(AsOptionalString(payload["query"]))
			payload["limit"] = MinInt(intFromAny(payload["limit"]), degradedRetrievePapersLimit)
		}
...
func (e *PlanExecutor) executeStepOnce(
	...
	tracer := telemetry.Tracer(fmt.Sprintf("scholarlm.%s.executor", namespace))
	...
	if reason, exceeded := stepExecutionBudgetExceeded(step, session.Budget); exceeded {
		...
		logStepHandoffFailure(ctx, "handoff_rejected", session, step, ownership, false,
			"result", "failure",
	...
	logStepHandoffStage(ctx, "guardrail_evaluated", session, step, ownership, degraded,
		"allowed", guard.Allowed,
		"requires_confirmation", guard.RequiresConfirmation,
		"reason", guard.Reason,
		"mode", string(NormalizeWisDevMode(string(session.Mode))),
	)
	if !guard.Allowed {
	...
	if guard.RequiresConfirmation && !approved {
		if shouldBypassConfirmationForAutonomousMode(session, step) {
			logStepHandoffStage(ctx, "handoff_confirmation_bypassed", session, step, ownership, false,
				"result", "allowed",
				"reason", guard.Reason,
				"bypass_reason", "yolo_internal_research_step",
			)
		} else {
			err := fmt.Errorf("CONFIRMATION_REQUIRED:%s", guard.Reason)
			span.RecordError(err)
			span.SetStatus(codes.Error, guard.Reason)
			logStepHandoffFailure(ctx, "handoff_confirmation_required", session, step, ownership, false,
				"result", "blocked",
	...
	if step.RequiresHumanCheckpoint && !approved {
		if shouldBypassConfirmationForAutonomousMode(session, step) {
			logStepHandoffStage(ctx, "handoff_confirmation_bypassed", session, step, ownership, false,
				"result", "allowed",
				"reason", "human checkpoint required",
				"bypass_reason", "yolo_internal_research_step",
			)
		} else {
	...
	logStepHandoffStage(ctx, "agent_query_handoff", session, step, ownership, degraded,
		"model", modelName,
	)
	...
	if delegated {
		...
		if err != nil {
			...
			logStepHandoffFailure(ctx, "agent_query_handoff_failed", session, step, ownership, degraded,
				"result", "failure",
				"error_code", "DELEGATED_STEP_FAILED",
				"error", err.Error(),
		...
		logStepHandoffStage(ctx, "agent_query_handoff_complete", session, step, ownership, degraded,
			"result", "success",
			"result_count", len(papers),
			"confidence", confidence,
		)
	...
	case ExecutionTargetGoNative:
		...
		if _, ok := basePayload["limit"]; !ok {
			basePayload["limit"] = defaultGoNativeStepLimit(step.Action)
		}
		if degraded {
			basePayload["query"] = degradedQuery(AsOptionalString(basePayload["query"]))
			basePayload["limit"] = MinInt(intFromAny(basePayload["limit"]), degradedRetrievePapersLimit)
		}
		...
		if err != nil {
			...
			logStepHandoffFailure(ctx, "agent_query_handoff_failed", session, step, ownership, degraded,
				"result", "failure",
				"error_code", "GO_NATIVE_STEP_FAILED",
				"error", err.Error(),
				"result_count", len(papers),
				"confidence", confidence,
			)
	...
	default:
		...
		logStepHandoffFailure(ctx, "agent_query_handoff_failed", session, step, ownership, degraded,
			"result", "failure",
			"error_code", "UNSUPPORTED_EXECUTION_TARGET",
			"error", err.Error(),
		)
		return nil, nil, 0, err
	}
...
func shouldBypassConfirmationForAutonomousMode(session *AgentSession, step PlanStep) bool {
	if session == nil || NormalizeWisDevMode(string(session.Mode)) != WisDevModeYOLO {
		return false
	}
	if step.ExecutionTarget == ExecutionTargetPythonSandbox {
		return false
