...
func TestPlanExecutorRetrievePapersUsesConfiguredMCPRegistry(t *testing.T) {
	...
	session := &AgentSession{
		OriginalQuery:  "paperclip retrieval",
		CorrectedQuery: "paperclip retrieval",
	...
	assert.Equal(t, "paperclip retrieval", provider.query)
...
func TestPlanExecutor_RunStepWithRecovery_YoloBypassesInternalResearchConfirmation(t *testing.T) {
	disableExecutorGuardrailDeps(t)
	...
	e := &PlanExecutor{
		...
		pythonExecute: func(ctx context.Context, action string, payload map[string]any, sess *AgentSession) (map[string]any, error) {
			return map[string]any{"success": true, "confidence": 0.91}, nil
		},
	}
	session := &AgentSession{
		SessionID: "yolo-confirmation-bypass",
		Mode:      WisDevModeYOLO,
		Status:    SessionExecutingPlan,
		Budget:    policy.BudgetState{MaxToolCalls: 10, MaxCostCents: 1000},
		Plan: &PlanState{
			PlanID:          "plan-yolo-confirmation-bypass",
			ApprovedStepIDs: make(map[string]bool),
		},
	}
	...
	for _, step := range []PlanStep{
		...
		result := e.RunStepWithRecovery(context.Background(), session, step, 1)
		require.NoError(t, result.Err)
		assert.Equal(t, step.ID, result.Step.ID)
		assert.Equal(t, 0.91, result.Confidence)
	}
}

func TestPlanExecutor_RunStepWithRecovery_GuidedStillRequiresInternalResearchConfirmation(t *testing.T) {
	disableExecutorGuardrailDeps(t)
	...
	session := &AgentSession{
		SessionID: "guided-confirmation-required",
		Mode:      WisDevModeGuided,
		Status:    SessionExecutingPlan,
		Budget:    policy.BudgetState{MaxToolCalls: 10, MaxCostCents: 1000},
		Plan: &PlanState{
			PlanID:          "plan-guided-confirmation-required",
			ApprovedStepIDs: make(map[string]bool),
		},
	}

	result := e.RunStepWithRecovery(context.Background(), session, PlanStep{
		ID:              "step-04",
		Action:          "research.resolveCanonicalCitations",
		Risk:            RiskLevelMedium,
		ExecutionTarget: ExecutionTargetPythonCapability,
	...
	assert.Contains(t, result.Err.Error(), "CONFIRMATION_REQUIRED:medium_risk_confirmation_required")
}
...
func TestPlanExecutor_Execute_CompletedEventCarriesRetrievedPapers(t *testing.T) {
	disableExecutorGuardrailDeps(t)
		...
		pythonExecute: func(ctx context.Context, action string, payload map[string]any, sess *AgentSession) (map[string]any, error) {
			return map[string]any{
				"papers": []any{
					map[string]any{
						"id":     "p-yolo-1",
						"title":  "YOLO Terminal Evidence",
						"doi":    "10.1000/yolo-terminal",
						"link":   "https://doi.org/10.1000/yolo-terminal",
						"source": "openalex",
					},
				},
			}, nil
	...
	session := &AgentSession{
		SessionID: "executor-terminal-papers",
		Status:    SessionGeneratingTree,
		Mode:      WisDevModeYOLO,
		Budget: policy.BudgetState{
			MaxToolCalls: 10,
		...
		Plan: &PlanState{
			PlanID: "plan-terminal-papers",
			Steps: []PlanStep{
				{
					ID:              "step-03",
					Action:          ActionResearchRetrievePapers,
					Reason:          "retrieve evidence first in YOLO mode",
					ExecutionTarget: ExecutionTargetPythonCapability,
					Risk:            RiskLevelLow,
				},
			},
	...
	var terminal *PlanExecutionEvent
	for ev := range out {
		if ev.Type == EventCompleted {
			tmp := ev
			terminal = &tmp
	...
	assert.Equal(t, 1, terminal.Payload["resultCount"])
	assert.Equal(t, []string{"step-03"}, toStringSlice(terminal.Payload["completedStepIds"]))
	papers := firstArtifactMaps(terminal.Payload["papers"])
	require.Len(t, papers, 1)
	assert.Equal(t, "YOLO Terminal Evidence", papers[0]["title"])
	assert.Equal(t, "10.1000/yolo-terminal", papers[0]["doi"])
}
