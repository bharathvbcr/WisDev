...
func TestAgentGateway_RuntimeMetadata(t *testing.T) {
	...
	t.Run("With Runtime", func(t *testing.T) {
		...
		if assert.True(t, ok) {
			...
			assert.Contains(t, artifactSchema["compatibilityKeys"], "claimEvidenceTable")
		}
	})
}
...
func TestAgentGatewayRetrievePapersUsesCoreMCPToolWhenRegistryConfigured(t *testing.T) {
	...
	assert.Equal(t, "paperclip mcp retrieval", provider.query)
...
func TestAgentGateway_ExecuteADKAction_AllRegisteredGoNativeTools(t *testing.T) {
	...
	mockLLM.On("StructuredOutput", mock.Anything, mock.MatchedBy(func(req *llmv1.StructuredRequest) bool {
		return req != nil && strings.Contains(req.Prompt, "Resolve canonical citations")
	})).Return(&llmv1.StructuredResponse{JsonResult: `{"resolved":[{"id":"p1","canonicalId":"10.1000/p1"}]}`}, nil).Maybe()
	mockLLM.On("StructuredOutput", mock.Anything, mock.MatchedBy(func(req *llmv1.StructuredRequest) bool {
	...
	payloadFor := func(action string) map[string]any {
		...
		case ActionResearchResolveCanonicalCitations:
			return map[string]any{
				"papers": []any{map[string]any{"id": "p1", "title": "Citation Paper", "doi": "10.1000/p1"}},
			}
		case ActionResearchVerifyReasoningPaths:
			return map[string]any{
				"branches": []any{map[string]any{
					...
					"findings": []any{
						map[string]any{"sourceId": "p1", "snippet": "supported"},
						map[string]any{"sourceId": "p2", "snippet": "replicated"},
					},
				}},
			}
	...
	for _, tool := range NewToolRegistry().List() {
		...
		t.Run(tool.Name, func(t *testing.T) {
			...
			case ActionResearchResolveCanonicalCitations:
				assert.NotEmpty(t, result["canonicalSources"])
			case ActionResearchVerifyReasoningPaths:
				assert.NotEmpty(t, result["results"])
			case ActionResearchSynthesizeAnswer:
