...
func TestAgentGateway_DefaultPythonExecutor(t *testing.T) {
	...
	t.Run("research.synthesizeAnswer", func(t *testing.T) {
		...
		res, err := gw.defaultPythonExecutor(ctx, ActionResearchSynthesizeAnswer, map[string]any{"query": "test", "papers": []any{}}, nil)
		assert.NoError(t, err)
...
func TestAgentGateway_ExecuteADKAction_ResolveCanonicalCitationsUsesGoTool(t *testing.T) {
	mockLLM := new(mockLLMServiceClient)
	client := llm.NewClient()
	client.SetClient(mockLLM)
	...
	gw := NewAgentGateway(nil, nil, nil)
	gw.Brain = NewBrainCapabilities(client)
	tool, err := gw.Registry.Get(ActionResearchResolveCanonicalCitations)
	require.NoError(t, err)
	require.Equal(t, ExecutionTargetGoNative, tool.ExecutionTarget)

	mockLLM.On("StructuredOutput", mock.Anything, mock.MatchedBy(func(req *llmv1.StructuredRequest) bool {
		return assertBrainRecoverableStructuredPolicy(t, req)
	})).Return(&llmv1.StructuredResponse{JsonResult: `{"resolved":[{"id":"p1","canonicalId":"10.1000/p1"}]}`}, nil).Once()

	res, err := gw.ExecuteADKAction(context.Background(), tool, map[string]any{
		"papers": []any{map[string]any{"id": "p1", "title": "Paper 1", "doi": "10.1000/p1"}},
	}, &AgentSession{SessionID: "s-canon", Query: "citation grounding"})

	require.NoError(t, err)
	canonical, ok := res["canonicalSources"].([]any)
	require.True(t, ok)
	require.Len(t, canonical, 1)
	assert.Equal(t, 1, res["resolvedCount"])
...
func TestAgentGateway_ProgrammaticLoopExecutorUsesOfficialADKRunnerWhenAvailable(t *testing.T) {
	gw := &AgentGateway{}
	gw.PythonExecute = func(ctx context.Context, action string, payload map[string]any, session *AgentSession) (map[string]any, error) {
		if payload["adkRunnerInternal"] != true {
			t.Fatalf("expected ADK runner internal dispatch guard, got %#v", payload)
		}
		return map[string]any{"action": action, "confidence": 0.83}, nil
	}
	gw.ADKRuntime = &ADKRuntime{Config: DefaultADKRuntimeConfig()}
	gw.ADKRuntime.delegateExecutor = gw.programmaticLoopExecutorDirect()

	result, err := gw.ProgrammaticLoopExecutor()(context.Background(), "research.queryDecompose", map[string]any{"query": "sleep memory"}, &AgentSession{
		SessionID: "adk-programmatic-session",
		UserID:    "user-1",
	})
