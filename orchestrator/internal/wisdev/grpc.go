...
func (s *agentGatewayGRPCServer) ExecutePlan(req *wisdevpb.ExecutePlanRequest, stream wisdevpb.AgentGateway_ExecutePlanServer) error {
	...
	if s.gateway.ADKRuntime != nil && s.gateway.ADKRuntime.Runner != nil && s.gateway.ADKRuntime.Agent != nil {
		go func() {
			defer close(ch)
			if err := s.gateway.ADKRuntime.ExecuteSession(stream.Context(), session, func(event PlanExecutionEvent) {
				select {
				case ch <- event:
...
func mapPlanExecutionEventToUpdate(event PlanExecutionEvent) *wisdevpb.PlanExecutionUpdate {
	...
	case EventConfirmationNeed:
		approvalToken := AsOptionalString(event.Payload["approvalToken"])
		allowedActions := toStringSlice(event.Payload["allowedActions"])
		if len(allowedActions) == 0 {
			allowedActions = ConfirmationActions()
		}
		action := firstNonEmpty(
			AsOptionalString(event.Payload["action"]),
			AsOptionalString(event.Payload["adkOriginalFunctionCallName"]),
			event.Message,
		)
		rationale := firstNonEmpty(
			AsOptionalString(event.Payload["rationale"]),
			AsOptionalString(event.Payload["adkConfirmationHint"]),
			event.Message,
		)
		stepID := firstNonEmpty(
			event.StepID,
			AsOptionalString(event.Payload["stepID"]),
			AsOptionalString(event.Payload["stepId"]),
		)
		guardrailReason := firstNonEmpty(
			AsOptionalString(event.Payload["guardrailReason"]),
			event.Message,
			AsOptionalString(event.Payload["adkConfirmationHint"]),
		)
		update.Update = &wisdevpb.PlanExecutionUpdate_ConfirmationNeeded{
			ConfirmationNeeded: &wisdevpb.ConfirmationRequired{
				...
				AdkConfirmationFunction:         AsOptionalString(event.Payload["adkConfirmationFunction"]),
				AdkConfirmationRequestId:        AsOptionalString(event.Payload["adkConfirmationRequestId"]),
				AdkOriginalFunctionCallId:       AsOptionalString(event.Payload["adkOriginalFunctionCallId"]),
				AdkOriginalFunctionCallName:     AsOptionalString(event.Payload["adkOriginalFunctionCallName"]),
				AdkConfirmationHint:             AsOptionalString(event.Payload["adkConfirmationHint"]),
				AdkConfirmationPayloadJson:      marshalPlanEventPayloadJSON(event.Payload["adkConfirmationPayload"]),
				AdkOriginalFunctionCallArgsJson: marshalPlanEventPayloadJSON(event.Payload["adkOriginalFunctionCallArgs"]),
			},
		}
	case EventProgress:
...
func marshalPlanEventPayloadJSON(value any) string {
	if value == nil {
		return ""
	}
