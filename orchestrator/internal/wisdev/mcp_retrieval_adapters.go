...
func retrieveCanonicalSearchResult(ctx context.Context, registry *search.ProviderRegistry, query string, opts search.SearchOpts) (search.SearchResult, error) {
	...
	if count, ok := intFromRetrievalPayload(payload["rawResultCount"]); ok {
		result.RawResultCount = count
	}
	if count, ok := intFromRetrievalPayload(payload["fusedResultCount"]); ok {
		result.FusedResultCount = count
	}
	if count, ok := intFromRetrievalPayload(payload["dedupedCount"]); ok {
		result.DedupedCount = count
	}
	if count, ok := intFromRetrievalPayload(payload["finalCount"]); ok {
		result.FinalCount = count
	}
	if applied, ok := boolFromRetrievalPayload(payload["limitApplied"]); ok {
		result.LimitApplied = applied
	}
	return result, err
}
...
func boolFromRetrievalPayload(value any) (bool, bool) {
	switch typed := value.(type) {
	case bool:
		return typed, true
	case string:
