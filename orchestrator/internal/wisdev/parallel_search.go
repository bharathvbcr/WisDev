...
func logParallelSearchStage(ctx context.Context, stage string, query string, attrs ...any) {
	telemetry.FromCtx(ctx).InfoContext(ctx, "wisdev parallel search lifecycle", buildParallelSearchLogArgs(stage, query, attrs...)...)
}

func buildParallelSearchLogArgs(stage string, query string, attrs ...any) []any {
	trimmedStage := strings.TrimSpace(stage)
	if trimmedStage == "" {
		trimmedStage = "unknown"
	}
	base := []any{
...
func executeWithResilience[T any](ctx context.Context, name string, cb *CircuitBreaker, sem *semaphore.Weighted, op func() (T, error)) (T, error) {
	var empty T
	if !sem.TryAcquire(1) {
		telemetry.FromCtx(ctx).WarnContext(ctx, "wisdev parallel search resilience rejected request",
			"service", "go_orchestrator",
			"runtime", "go",
			"component", "wisdev.parallel_search",
			"operation", "resilience_guard",
			"stage", "semaphore_shedding",
...
func searchSemanticScholar(ctx context.Context, query string, limit int) ([]Source, error) {
	return directProviderSearchDisabled(ctx, "Semantic Scholar", query, limit)
}
func searchOpenAlex(ctx context.Context, query string, limit int) ([]Source, error) {
	return directProviderSearchDisabled(ctx, "OpenAlex", query, limit)
}
func searchPubMed(ctx context.Context, query string, limit int) ([]Source, error) {
	return directProviderSearchDisabled(ctx, "PubMed", query, limit)
}
func searchCORE(ctx context.Context, query string, limit int) ([]Source, error) {
	return directProviderSearchDisabled(ctx, "CORE", query, limit)
}
func searchArXiv(ctx context.Context, query string, limit int) ([]Source, error) {
	return directProviderSearchDisabled(ctx, "arXiv", query, limit)
}

func directProviderSearchDisabled(ctx context.Context, provider string, _ string, _ int) ([]Source, error) {
	if ctx != nil {
...
// its result into the WisDev search surface.
