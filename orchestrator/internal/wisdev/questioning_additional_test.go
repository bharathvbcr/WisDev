...
func TestQuestioningHelpers(t *testing.T) {
	t.Run("question index and complexity", func(t *testing.T) {
		...
		for _, question := range DefaultQuestionFlow() {
			assert.NotEmpty(t, question.HelpText, question.ID)
			for _, option := range question.Options {
				assert.NotEmpty(t, option.Label, question.ID)
				assert.NotEmpty(t, option.Description, question.ID+"."+option.Value)
			}
		}
