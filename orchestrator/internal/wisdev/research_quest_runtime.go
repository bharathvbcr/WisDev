...
type CitationBrokerGateConfig struct {
	...
	InternalServiceKeyConfigured bool     `json:"internalServiceKeyConfigured"`
...
func (cfg CitationBrokerGateConfig) Map() map[string]any {
	return map[string]any{
		...
		"internalServiceKeyConfigured": cfg.InternalServiceKeyConfigured,
...
func (rt *ResearchQuestRuntime) StartQuest(ctx context.Context, req ResearchQuestRequest) (*ResearchQuest, error) {
	...
	quest := &ResearchQuest{
		...
		ReferenceInspiration:   NormalizeReferenceResearchInspirationPlan(req),
		Events:                 []QuestEvent{},
	...
	rt.appendEvent(quest, "quest_started", QuestStageInit, "Research quest started", map[string]any{
		...
		"kickoffMode":      quest.ReferenceInspiration.KickoffMode,
		"referenceSignals": append([]string(nil), quest.ReferenceInspiration.ReplaySignals...),
	})
...
func (rt *ResearchQuestRuntime) RunReferenceDoctorPreflight(ctx context.Context, questID string, opts ...ResearchDoctorPreflightOptions) (*ResearchQuest, error) {
	quest, err := rt.LoadQuest(ctx, questID)
	if err != nil {
		return nil, err
	}
	...
	SyncReferenceResearchInspirationArtifact(quest)
	preflightOptions := ResearchDoctorPreflightOptions{}
	if len(opts) > 0 {
		preflightOptions = opts[0]
	}
	RunReferenceDoctorPreflight(ctx, quest.ReferenceInspiration, preflightOptions)
	SyncReferenceResearchInspirationArtifact(quest)
	rt.appendEvent(quest, "reference_doctor_preflight", QuestStageInit, "Reference research doctor preflight completed.", map[string]any{
		"ready":          quest.ReferenceInspiration.DoctorPreflight.Ready,
		"blockingIssues": append([]string(nil), quest.ReferenceInspiration.DoctorPreflight.BlockingIssues...),
		"executionGate":  quest.ReferenceInspiration.ExperimentExecutionGate.Status,
	})
	if err := rt.persistQuest(ctx, quest); err != nil {
		return nil, err
	}
	...
	if rt.gateway != nil && rt.gateway.ResearchMemory != nil {
		...
		if quest.ReferenceInspiration != nil {
			...
			episodeInput.Metadata = map[string]any{
				"referenceResearchInspiration": quest.ReferenceInspiration,
				"referenceSignals":             append([]string(nil), quest.ReferenceInspiration.ReplaySignals...),
				"baselineRunArtifact":          quest.ReferenceInspiration.BaselineRunArtifact,
				"candidateRunArtifact":         quest.ReferenceInspiration.CandidateRunArtifact,
				"metricDeltas":                 append([]ResearchMetricDelta(nil), quest.ReferenceInspiration.MetricDeltas...),
				"failedPathMemory":             append([]FailedPathMemoryEntry(nil), quest.ReferenceInspiration.FailedPathMemory...),
				"innovationGraph":              quest.ReferenceInspiration.InnovationGraph,
				"reviewImprovementDelta":       quest.ReferenceInspiration.ReviewImprovementDelta,
			}
		}
		...
		if reasoningBankEnabled() && rt.gateway != nil && rt.gateway.Brain != nil {
			...
			if err != nil {
				...
				if episodeInput.Metadata == nil {
					episodeInput.Metadata = map[string]any{}
				}
				episodeInput.Metadata["judgeScore"] = judgeOutput.Score
				episodeInput.Metadata["judgeOutcome"] = string(judgeOutput.Outcome)
				episodeInput.Metadata["successFactors"] = judgeOutput.SuccessFactors
				episodeInput.Metadata["failureFactors"] = judgeOutput.FailureFactors
				episodeInput.Metadata["judgeReasoning"] = judgeOutput.Reasoning
...
func buildQuestCanonicalRuntimePayload(result *UnifiedResearchResult, querySeed string, followUpQueries []string) map[string]any {
	...
	if result.LoopResult != nil {
		...
		if len(result.LoopResult.ReasoningTrace) > 0 {
			payload["reasoningTrace"] = valueToAny(result.LoopResult.ReasoningTrace)
		}
		if review := buildLoopAutonomousReviewMetadata(result.LoopResult); review != nil {
...
func ResolveCitationBrokerGateConfig() CitationBrokerGateConfig {
	gatewayURL := strings.TrimRight(strings.TrimSpace(os.Getenv("RUST_GATEWAY_INTERNAL_URL")), "/")
	allowGoFallback := strings.EqualFold(strings.TrimSpace(os.Getenv(allowGoCitationFallbackEnv)), "true")
	config := CitationBrokerGateConfig{
		GatewayURL:                   gatewayURL,
		Mode:                         "strict",
		AllowGoFallback:              allowGoFallback,
		InternalServiceKeyConfigured: stackconfig.ResolveInternalServiceKey() != "",
	}
	if allowGoFallback {
		config.Mode = "emergency_go_fallback"
		config.Warnings = []string{"Go citation fallback enabled"}
	}
	return config
}

func ResolveCitationsLive(papers []Source) ([]map[string]any, error) {
	envelope, err := resolveCitationsLiveEnvelope(papers)
	if err != nil {
		return nil, err
	}
	return normalizeResolvedCitationRecords(envelope.Data.Resolved, envelope.Data.Engine), nil
}

func VerifyCitationRecordsSecurely(papers []Source) (map[string]any, error) {
	envelope, err := resolveCitationsLiveEnvelope(papers)
	if err == nil {
		return buildCitationVerificationPayload(
			normalizeResolvedCitationRecords(envelope.Data.Resolved, envelope.Data.Engine),
			envelope.Data.ResolverTrace,
			envelope.Data.BlockingIssues,
			firstNonEmpty(strings.TrimSpace(envelope.Data.Engine), "rust-citation-authority-broker"),
			envelope.Data.PromotionEligible,
		), nil
	}

	config := ResolveCitationBrokerGateConfig()
	if !config.AllowGoFallback {
		return nil, fmt.Errorf("%w: go fallback disabled", err)
	}
	return buildCitationVerificationPayload(localCitationFallbackRecords(papers), nil, nil, "go-fallback", false), nil
}

func resolveCitationsLiveEnvelope(papers []Source) (*citationResolveEnvelope, error) {
	config := ResolveCitationBrokerGateConfig()
	if strings.TrimSpace(config.GatewayURL) == "" {
		return nil, fmt.Errorf("citation gateway url is not configured")
	}

	requestItems := make([]map[string]any, 0, len(papers))
	for _, paper := range papers {
	...
	body, err := json.Marshal(map[string]any{"items": requestItems})
	if err != nil {
		return nil, err
	}

	req, err := http.NewRequest(http.MethodPost, config.GatewayURL+"/internal/citations/resolve", bytes.NewReader(body))
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("citation resolve request failed: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("citation resolve returned status %d", resp.StatusCode)
	}

	var envelope citationResolveEnvelope
	if err := json.NewDecoder(resp.Body).Decode(&envelope); err != nil {
		return nil, err
	}
	if !envelope.OK {
		if strings.TrimSpace(envelope.Error) != "" {
			return nil, errors.New(strings.TrimSpace(envelope.Error))
		}
		return nil, fmt.Errorf("citation resolve failed")
	}
	return &envelope, nil
}
...
func normalizeResolvedCitationRecords(records []map[string]any, engine string) []map[string]any {
	...
	for _, record := range records {
		...
		canonical := canonicalCitationWithTrustDefaults(CanonicalCitation{
			...
			CitationCount:          toInt(firstNonEmptyValue(record["citationCount"], record["citation_count"])),
			Resolved:               resolved,
			Verified:               verified,
...
func buildCitationVerificationPayload(records []map[string]any, resolverTrace []map[string]any, issues []string, engine string, promotionEligible bool) map[string]any {
	...
	return map[string]any{
		"engine":              firstNonEmpty(strings.TrimSpace(engine), "go-fallback"),
...
func normalizeDuplicateCitationAuthorities(records []CanonicalCitation) []CanonicalCitation {
	...
	for idx, record := range normalized {
		...
		if _, ok := seen[key]; ok {
			...
			normalized[idx].ResolutionEngine = firstNonEmpty(normalized[idx].ResolutionEngine, "rust-citation-authority-broker")
