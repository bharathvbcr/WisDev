...
func TestResearchQuestRuntimeResumeRecoversAfterCitationBrokerOutage(t *testing.T) {
	server, available := startChaosCitationResolveServer(t)
	defer server.Close()
	...
	require.NoError(t, err)
	assert.Equal(t, QuestStageVerify, quest.CurrentStage)
	assert.Equal(t, QuestStatusRunning, quest.Status)
	assert.Equal(t, 1, retrieveCalls)
	assert.Contains(t, quest.Events[len(quest.Events)-1].Summary, "Citation verification failed")
...
func TestResearchQuestRuntimeConcurrentResumeRecoversAfterBrokerRestoration(t *testing.T) {
	server, available := startChaosCitationResolveServer(t)
	defer server.Close()
	...
	for idx := 0; idx < 4; idx++ {
		...
		require.NoError(t, err)
		require.Equal(t, QuestStageVerify, quest.CurrentStage)
	...
	for idx := range runtimes {
		...
		go func(runtime *ResearchQuestRuntime, questID string) {
			...
			if resumed.Status != QuestStatusComplete || !resumed.CitationVerdict.Promoted {
				errCh <- fmt.Errorf("quest %s did not recover cleanly", questID)
			}
...
func startChaosCitationResolveServer(t *testing.T) (*httptest.Server, *atomic.Bool) {
	t.Helper()

	var available atomic.Bool
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/internal/citations/resolve" {
			http.NotFound(w, r)
			return
		}
		if !available.Load() {
		...
		for _, item := range req.Items {
			...
			resolved = append(resolved, map[string]any{
				...
				"title":                    title,
				"year":                     2025,
				"authors":                  []string{"ScholarLM"},
				"engine":                   "rust-citation-authority-broker",
				"api_confirmed":            doi != "",
