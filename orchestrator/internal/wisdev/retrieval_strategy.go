...
const (
	...
	defaultRetrievePapersLimit          = 50
	degradedRetrievePapersLimit         = 5
)
...
func defaultPageIndexRerank(mode WisDevMode, tier ServiceTier) bool {
	return tier == ServiceTierFlex || tier == ServiceTierPriority
}

func defaultStage2Rerank(mode WisDevMode, tier ServiceTier) bool {
	return tier == ServiceTierPriority
}
...
func defaultRetrievePapersPlanParams(session *AgentSession) map[string]any {
	...
	return map[string]any{
		...
		"domain":              domain,
		"limit":               defaultRetrievePapersLimit,
...
func resolveRetrievePapersSearchOptions(payload map[string]any, session *AgentSession, degraded bool) (string, SearchOptions) {
	...
	if degraded && limit > degradedRetrievePapersLimit {
		limit = degradedRetrievePapersLimit
	}
...
func runMCPPaperRetrieval(ctx context.Context, registry *internalsearch.ProviderRegistry, query string, opts SearchOptions) ([]Source, map[string]any, error) {
	...
	if bundle, ok := out["paperBundle"].(map[string]any); ok {
		...
		bundle["rawResultCount"] = result.RawResultCount
		bundle["fusedResultCount"] = result.FusedResultCount
		bundle["dedupedCount"] = result.DedupedCount
		bundle["finalCount"] = result.FinalCount
		bundle["limitApplied"] = result.LimitApplied
...
// It enforces the canonical WisDev retrieval contract for callers that only
// provide a query and a paper limit.
