...
func TestSelfHealer_Execute_UsesOfficialADKRunnerWhenAvailable(t *testing.T) {
	mLLM := new(mockLLM)
	mExec := new(mockExecutor)
	sh := NewSelfHealer(mLLM, mExec).WithADKRuntime(&ADKRuntime{Config: DefaultADKRuntimeConfig()})

	step := PlanStep{ID: "step_adk_self_heal", Action: "search.arxiv"}
	mExec.On("RunStepWithRecovery", mock.Anything, mock.MatchedBy(func(session *AgentSession) bool {
		return session != nil && session.SessionID == "sess-adk-self-heal"
	}), step, 1).Return(StepResult{Sources: []Source{{ID: "src-adk"}}}).Once()

	result, err := sh.Execute(context.Background(), "sess-adk-self-heal", step)
