...
func (rt *UnifiedResearchRuntime) RunLoop(
	...
	session := &AgentSession{
		...
		ServiceTier:    ResolveLoopServiceTier(NormalizeWisDevMode(req.Mode), false, req.ServiceTier),
...
func buildVerifierControlledFollowUpQueries(
	...
	if verifierDecisionNeedsAbstentionRecovery(decision) {
		for _, candidate := range verifierAbstentionRecoveryQueries(query, decision) {
			addCandidate(candidate, 1180+scoreFollowUpQueryText(candidate))
		}
	}
...
func verifierDecisionNeedsAbstentionRecovery(decision *ResearchVerifierDecision) bool {
	if decision == nil {
		return false
	}
	if !strings.EqualFold(strings.TrimSpace(decision.Verdict), "abstain") && !strings.EqualFold(strings.TrimSpace(decision.StopReason), "verifier_abstained") {
		return false
	}
	return len(decision.PromotedClaimIDs) == 0
}

func verifierAbstentionRecoveryQueries(query string, decision *ResearchVerifierDecision) []string {
	baseQuery := strings.TrimSpace(firstNonEmpty(query, "research question"))
	queries := []string{
		baseQuery + " canonical DOI OpenAlex Semantic Scholar source identity independent source family",
		baseQuery + " independent replication systematic review counter evidence",
		baseQuery + " open access PDF full text citation metadata",
	}
	if decision != nil {
		for _, reason := range decision.RevisionReasons {
			lower := strings.ToLower(strings.TrimSpace(reason))
			switch {
			case strings.Contains(lower, "contradict") || strings.Contains(lower, "counter"):
				queries = append(queries, baseQuery+" contradiction replication null result")
			case strings.Contains(lower, "citation") || strings.Contains(lower, "single-source") || strings.Contains(lower, "source"):
				queries = append(queries, baseQuery+" DOI citation metadata source family triangulation")
			case strings.Contains(lower, "claim remains") || strings.Contains(lower, "unverified"):
				queries = append(queries, baseQuery+" claim entailment independent corroborating evidence")
			}
		}
	}
...
func buildVerifierRevisionLoopRequest(baseReq LoopRequest, loopResult *LoopResult, followUpQueries []string, budget *ResearchBudgetDecision) LoopRequest {
	req := baseReq
	combinedSeeds := verifierRevisionSeedQueries(baseReq.Query, baseReq.SeedQueries, followUpQueries)
	...
	additionalSearches := len(normalizeLoopQueries("", followUpQueries))
	if budget != nil && budget.FollowUpSearchBudget > 0 {
...
func verifierRevisionSeedQueries(query string, baseSeeds []string, followUpQueries []string) []string {
	ordered := make([]string, 0, 1+len(followUpQueries)+len(baseSeeds))
	root := strings.TrimSpace(query)
	if root != "" {
		ordered = append(ordered, root)
	}
	ordered = append(ordered, normalizeLoopQueries("", followUpQueries)...)
	for _, seed := range normalizeLoopQueries("", baseSeeds) {
		if root != "" && strings.EqualFold(strings.TrimSpace(seed), root) {
			continue
		}
		if containsNormalizedLoopQuery(ordered, seed) {
			continue
		}
...
func (rt *UnifiedResearchRuntime) RunAnswer(ctx context.Context, req UnifiedResearchRequest) (*rag.AnswerResponse, error) {
	...
	finalStopReason := strings.TrimSpace(firstNonEmpty(looped.State.StopReason, loopResult.StopReason))
	if finalizationGate != nil {
		finalStopReason = strings.TrimSpace(firstNonEmpty(finalizationGate.StopReason, finalStopReason))
	}
	answerVerified := finalizationGate != nil && finalizationGate.Ready && !finalizationGate.Provisional
	answerStatus := ResearchAnswerStatusFromState(looped.State, finalizationGate, answerVerified, finalStopReason)
	verifierVerdict := ""
	if finalizationGate != nil {
		verifierVerdict = strings.TrimSpace(finalizationGate.VerifierVerdict)
	}
	return &rag.AnswerResponse{
		...
		Metadata: &rag.ResponseMetadata{
			...
			Policy: map[string]any{
				...
				"reasoningTrace":         loopResult.ReasoningTrace,
				"autonomousReview":       buildLoopAutonomousReviewMetadata(loopResult),
				"finalizationGate":       finalizationGate,
				"answerProvisional":      finalizationGate != nil && finalizationGate.Provisional,
				"answerVerified":         answerVerified,
				"answerStatus":           answerStatus,
				"finalizationStopReason": finalStopReason,
...
func BuildResearchReasoningRuntimeMetadata(req LoopRequest, plane ResearchExecutionPlane, budget *ResearchBudgetDecision) map[string]any {
	...
	return map[string]any{
		...
		"dynamicProviderSelectionEligible": plane == ResearchExecutionPlaneDeep || plane == ResearchExecutionPlaneMultiAgent,
	}
}
...
func buildIndependentVerifierDecision(claims *ClaimVerificationLedger, gap *LoopGapState, branches []ResearchBranchEvaluation) *ResearchVerifierDecision {
	...
	if claims.ContradictedClaims > 0 || (len(decision.PromotedClaimIDs) == 0 && len(decision.RejectedClaimIDs) > 0) {
		...
		if len(decision.PromotedClaimIDs) == 0 && len(decision.RejectedClaimIDs) > 0 {
			decision.RevisionReasons = append(decision.RevisionReasons, "no claim survived independent verification; require citation identity, full-text, and independent replication retrieval")
			decision.RevisionReasons = dedupeTrimmedStrings(decision.RevisionReasons)
		}
	}
	return decision
