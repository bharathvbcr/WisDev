...
func TestUnifiedResearchRuntimePayloadExposesAutonomousReviewTrace(t *testing.T) {
	result := &UnifiedResearchResult{
		LoopResult: &LoopResult{
			FinalAnswer: "Sleep interventions may improve memory.",
			StopReason:  "coverage_satisfied",
			Papers: []search.Paper{{
				ID:       "paper-1",
				Title:    "Sleep Intervention",
				Abstract: "Intervention evidence.",
				Source:   "pubmed",
			}},
	...
	payload := BuildUnifiedResearchRuntimePayload(result, "sleep memory", nil)

	trace, ok := payload["reasoningTrace"].([]any)
	if !ok || len(trace) != 2 {
		t.Fatalf("expected top-level reasoning trace, got %#v", payload["reasoningTrace"])
	}
	review, ok := payload["autonomousReview"].(map[string]any)
	if !ok {
		t.Fatalf("expected autonomousReview metadata, got %#v", payload["autonomousReview"])
	}
	if review["status"] != "resolved_after_follow_up" || review["postCritiqueReview"] != true {
		t.Fatalf("expected resolved post-critique review metadata, got %#v", review)
	}
	if review["stopReason"] != "coverage_satisfied" || review["openLedgerCount"] != 0 {
		t.Fatalf("expected closed review stop metadata, got %#v", review)
	}
	loopPayload, ok := payload["loopResult"].(map[string]any)
	if !ok {
		t.Fatalf("expected loopResult payload, got %#v", payload["loopResult"])
	}
	if _, ok := loopPayload["reasoningTrace"].([]any); !ok {
		t.Fatalf("expected nested loop reasoning trace to remain present, got %#v", loopPayload["reasoningTrace"])
	}
}
...
func TestExecuteResearchWorkersProducesExecutableRoleQueries(t *testing.T) {
	...
	queries := rt.executeResearchWorkers(context.Background(), state, session, state.Query, state.Domain, string(WisDevModeYOLO), true, 5, false)
...
func TestExecuteResearchWorkersHonorsDisabledProgrammaticPlanning(t *testing.T) {
	...
	queries := rt.executeResearchWorkers(context.Background(), state, session, state.Query, state.Domain, string(WisDevModeGuided), false, 5, false)
...
func TestExecuteResearchWorkersRunsBoundedSpecialistSearches(t *testing.T) {
	...
	reg.Register(&mockSearchProvider{
		...
		SearchFunc: func(ctx context.Context, query string, opts search.SearchOpts) ([]search.Paper, error) {
			...
			seenSkipCache = append(seenSkipCache, opts.SkipCache)
			mu.Unlock()
			return []search.Paper{{
	...
	queries := rt.executeResearchWorkers(context.Background(), state, session, state.Query, state.Domain, string(WisDevModeYOLO), false, 5, false, func(event PlanExecutionEvent) {
	...
	for _, skipCache := range seenSkipCache {
		if skipCache {
			t.Fatalf("expected worker searches to use cache by default, got SkipCache=true in %#v", seenSkipCache)
		}
	}
...
func TestExecuteResearchWorkersPropagatesSearchCacheBypass(t *testing.T) {
	reg := search.NewProviderRegistry()
	seenSkipCache := make([]bool, 0)
	reg.Register(&mockSearchProvider{
		name: "mock-worker-cache-bypass",
		SearchFunc: func(ctx context.Context, query string, opts search.SearchOpts) ([]search.Paper, error) {
			seenSkipCache = append(seenSkipCache, opts.SkipCache)
			return []search.Paper{{
				ID:       stableWisDevID("paper", query),
				...
				Source:   "mock-worker-cache-bypass",
				Score:    0.88,
			}}, nil
		},
	})
	rt := NewUnifiedResearchRuntime(nil, reg, nil, nil)
	state := newResearchSessionState("cache bypass propagation", "", "sess-worker-cache-bypass", ResearchExecutionPlaneSimple)
	session := &AgentSession{SessionID: state.SessionID, Query: state.Query}

	rt.executeResearchWorkers(context.Background(), state, session, state.Query, state.Domain, string(WisDevModeGuided), false, 1, true)

	if len(seenSkipCache) == 0 {
		t.Fatalf("expected worker search to execute")
	}
	for _, skipCache := range seenSkipCache {
		if !skipCache {
			t.Fatalf("expected worker searches to honor explicit cache bypass, got %#v", seenSkipCache)
		}
	}
}
...
func TestExecuteResearchWorkersAnchorsScoutOnRootQueryAndHonorsBudget(t *testing.T) {
	...
	rt.executeResearchWorkers(context.Background(), state, session, state.Query, state.Domain, string(WisDevModeGuided), true, 1, false)
...
func TestVerifierAbstentionAddsCitationIdentityRecoveryQueries(t *testing.T) {
	claims := &ClaimVerificationLedger{
	...
	decision := buildIndependentVerifierDecision(claims, nil, nil)
	if decision.Verdict != "abstain" || decision.StopReason != "verifier_abstained" {
		t.Fatalf("expected verifier to abstain when no claim survives, got %#v", decision)
	}
	if !containsQueryFragment(decision.RevisionReasons, "no claim survived independent verification") {
		t.Fatalf("expected abstention to explain recovery need, got %#v", decision.RevisionReasons)
	}
	...
	for _, expected := range []string{
		"canonical DOI OpenAlex Semantic Scholar source identity",
		"independent replication systematic review counter evidence",
		"open access PDF full text citation metadata",
	} {
		if !containsQueryFragment(queries, expected) {
			t.Fatalf("expected abstention recovery query containing %q, got %#v", expected, queries)
		}
	}
}
...
func TestBuildVerifierRevisionLoopRequestPrioritizesVerifierRecoverySeeds(t *testing.T) {
	base := LoopRequest{
		Query:          "sleep memory",
		SeedQueries:    []string{"sleep memory", "broad background", "general validation"},
		MaxIterations:  1,
		MaxSearchTerms: 1,
		HitsPerSearch:  2,
	}
	...
	req := buildVerifierRevisionLoopRequest(
		base,
		&LoopResult{ExecutedQueries: []string{"sleep memory"}},
		[]string{
			"sleep memory canonical DOI OpenAlex Semantic Scholar source identity",
			"sleep memory independent replication systematic review counter evidence",
		},
		&ResearchBudgetDecision{FollowUpSearchBudget: 2, CritiqueFollowUpLimit: 2},
	)

	if len(req.SeedQueries) < 5 {
		t.Fatalf("expected verifier recovery and prior seeds to be retained, got %#v", req.SeedQueries)
	}
	if req.SeedQueries[0] != "sleep memory" {
		t.Fatalf("expected root query to remain first, got %#v", req.SeedQueries)
	}
	if !containsQueryFragment(req.SeedQueries[1:3], "canonical DOI OpenAlex Semantic Scholar source identity") ||
		!containsQueryFragment(req.SeedQueries[1:3], "independent replication systematic review counter evidence") {
		t.Fatalf("expected verifier recovery seeds immediately after root query, got %#v", req.SeedQueries)
	}
	if containsQueryFragment(req.SeedQueries[:3], "broad background") || containsQueryFragment(req.SeedQueries[:3], "general validation") {
		t.Fatalf("generic prior seeds must not outrank verifier recovery seeds, got %#v", req.SeedQueries)
	}
	if req.MaxSearchTerms != 3 {
		t.Fatalf("expected revision search budget to count only verifier follow-ups, got %d", req.MaxSearchTerms)
	}
}
...
func TestResearchReasoningRuntimeMetadataTreatsJobPlaneAsHighDepth(t *testing.T) {
	req := LoopRequest{
		Query:                       "sleep memory",
	...
	budget := buildResearchBudgetDecision(req, ResearchExecutionPlaneJob)

	metadata := buildResearchReasoningRuntimeMetadata(req, ResearchExecutionPlaneJob, budget)

	if got := metadata["runtimeMode"]; got != "tree_search_with_programmatic_planner" {
		t.Fatalf("expected streamed job plane to use tree search planner mode, got %#v", got)
	}
	if got := metadata["treeSearchRuntime"]; got != true {
		t.Fatalf("expected job treeSearchRuntime=true, got %#v", got)
	}
	if got := metadata["programmaticTreePlanner"]; got != true {
		t.Fatalf("expected job programmaticTreePlanner=true, got %#v", got)
	}
	if got := metadata["mode"]; got != WisDevModeYOLO {
		t.Fatalf("expected yolo mode metadata, got %#v", got)
	}
	if got := metadata["researchPlane"]; got != ResearchExecutionPlaneJob {
		t.Fatalf("expected job research plane metadata, got %#v", got)
	}
}

func TestResearchReasoningRuntimeMetadataYoloKeepsGuidedStableAutonomousProviderEligibility(t *testing.T) {
	guided := BuildResearchReasoningRuntimeMetadata(LoopRequest{
		Query: "sleep memory",
		Mode:  string(WisDevModeGuided),
	}, ResearchExecutionPlaneAutonomous, nil)
	yolo := BuildResearchReasoningRuntimeMetadata(LoopRequest{
		Query: "sleep memory",
		Mode:  string(WisDevModeYOLO),
	}, ResearchExecutionPlaneAutonomous, nil)

	if got := yolo["dynamicProviderSelectionEligible"]; got != guided["dynamicProviderSelectionEligible"] {
		t.Fatalf("expected yolo autonomous dynamic provider eligibility to match guided, got yolo=%#v guided=%#v", got, guided["dynamicProviderSelectionEligible"])
	}
	if got := yolo["dynamicProviderSelectionEligible"]; got != false {
		t.Fatalf("expected autonomous dynamic provider selection to stay disabled, got %#v", got)
	}
}

func TestProgrammaticPlannerPayloadYoloUsesGuidedStableBranchEnvelope(t *testing.T) {
	guidedQueries := captureFirstProgrammaticPlannerPayload(t, func(rt *UnifiedResearchRuntime, session *AgentSession) {
		rt.planProgrammaticQueries(context.Background(), session, "sleep memory", "neuroscience", string(WisDevModeGuided))
	})
	yoloQueries := captureFirstProgrammaticPlannerPayload(t, func(rt *UnifiedResearchRuntime, session *AgentSession) {
		rt.planProgrammaticQueries(context.Background(), session, "sleep memory", "neuroscience", string(WisDevModeYOLO))
	})
	if got, want := yoloQueries["branchWidth"], guidedQueries["branchWidth"]; got != want {
		t.Fatalf("expected yolo query planner branchWidth to match guided, got %#v want %#v", got, want)
	}
	if _, ok := yoloQueries["maxDepth"]; ok {
		t.Fatalf("expected yolo query planner to avoid yolo-only maxDepth override, got payload %#v", yoloQueries)
	}

	guidedBranches := captureFirstProgrammaticPlannerPayload(t, func(rt *UnifiedResearchRuntime, session *AgentSession) {
		rt.planProgrammaticBranches(context.Background(), session, "sleep memory", "neuroscience", string(WisDevModeGuided))
	})
	yoloBranches := captureFirstProgrammaticPlannerPayload(t, func(rt *UnifiedResearchRuntime, session *AgentSession) {
		rt.planProgrammaticBranches(context.Background(), session, "sleep memory", "neuroscience", string(WisDevModeYOLO))
	})
	if got, want := yoloBranches["branchWidth"], guidedBranches["branchWidth"]; got != want {
		t.Fatalf("expected yolo branch planner branchWidth to match guided, got %#v want %#v", got, want)
	}
	if _, ok := yoloBranches["maxDepth"]; ok {
		t.Fatalf("expected yolo branch planner to avoid yolo-only maxDepth override, got payload %#v", yoloBranches)
	}
}

func captureFirstProgrammaticPlannerPayload(t *testing.T, invoke func(*UnifiedResearchRuntime, *AgentSession)) map[string]any {
	t.Helper()
	var captured map[string]any
	rt := NewUnifiedResearchRuntime(nil, nil, nil, func(ctx context.Context, action string, payload map[string]any, session *AgentSession) (map[string]any, error) {
		if captured == nil {
			captured = copyRuntimePayloadForTest(payload)
		}
		return map[string]any{
			"tasks": []map[string]any{{
				"query": "sleep memory evidence",
			}},
			"confidence": 0.9,
		}, nil
	})
	invoke(rt, &AgentSession{SessionID: "planner-payload", Query: "sleep memory", OriginalQuery: "sleep memory"})
	if captured == nil {
		t.Fatalf("expected programmatic planner to invoke runtime executor")
	}
	return captured
}

func copyRuntimePayloadForTest(payload map[string]any) map[string]any {
...
func TestRunAnswerExposesFinalizationGateMetadata(t *testing.T) {
	...
	if trace, ok := policy["reasoningTrace"].([]ReasoningTraceEntry); !ok || len(trace) == 0 {
		t.Fatalf("expected answer metadata to expose reasoning trace, got %#v", policy["reasoningTrace"])
	}
	if _, ok := policy["draftCritique"].(*LoopDraftCritique); !ok {
		t.Fatalf("expected answer metadata to expose draft critique, got %#v", policy["draftCritique"])
	}
	if review, ok := policy["autonomousReview"].(map[string]any); !ok || review["status"] == "" {
		t.Fatalf("expected answer metadata to expose autonomous review summary, got %#v", policy["autonomousReview"])
	}
