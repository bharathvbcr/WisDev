...
func (wa *WisDevWorkflowAgent) mapToADKEvent(ctx agent.InvocationContext, event PlanExecutionEvent) *session.Event {
	...
	case EventConfirmationNeed:
		return wa.mapConfirmationToADKEvent(ctx, adkEvent, event)
...
func (wa *WisDevWorkflowAgent) mapConfirmationToADKEvent(ctx agent.InvocationContext, adkEvent *session.Event, event PlanExecutionEvent) *session.Event {
	payload := workflowEventMetadata(event)
	if payload == nil {
		payload = map[string]any{}
	}
	hint := fmt.Sprintf("Confirmation required: %s", event.Message)
	confirmation := toolconfirmation.ToolConfirmation{
		Hint:    hint,
		Payload: cloneAnyMap(payload),
	}
	functionCallID := confirmationFunctionCallID(event, payload)
	originalFunctionCall := &genai.FunctionCall{
		ID:   functionCallID,
		Name: confirmationActionName(event, payload),
		Args: cloneAnyMap(payload),
	}
	requestConfirmation := &genai.FunctionCall{
		ID:   confirmationRequestID(event, payload),
	...
	adkEvent.Actions.RequestedToolConfirmations = map[string]toolconfirmation.ToolConfirmation{
		functionCallID: confirmation,
	}
	adkEvent.Actions.StateDelta = cloneAnyMap(payload)
	adkEvent.Actions.SkipSummarization = true
	adkEvent.LongRunningToolIDs = []string{requestConfirmation.ID}
	return adkEvent
}

func confirmationFunctionCallID(event PlanExecutionEvent, payload map[string]any) string {
	for _, key := range []string{"approvalId", "stepId"} {
		if value := strings.TrimSpace(AsOptionalString(payload[key])); value != "" {
			return value
		}
	}
	if value := strings.TrimSpace(event.StepID); value != "" {
		return value
	}
	return "wisdev_confirmation"
}

func confirmationRequestID(event PlanExecutionEvent, payload map[string]any) string {
	if value := strings.TrimSpace(AsOptionalString(payload["approvalToken"])); value != "" {
		return value
	}
	if value := strings.TrimSpace(AsOptionalString(payload["approvalId"])); value != "" {
		return value + "_request"
	}
	if value := strings.TrimSpace(event.StepID); value != "" {
		return value + "_request"
	}
	return "wisdev_confirmation_request"
}

func confirmationActionName(event PlanExecutionEvent, payload map[string]any) string {
	if value := strings.TrimSpace(AsOptionalString(payload["action"])); value != "" {
		return value
	}
	if value := strings.TrimSpace(event.StepID); value != "" {
		return value
	}
	return "wisdev_confirmed_action"
}
