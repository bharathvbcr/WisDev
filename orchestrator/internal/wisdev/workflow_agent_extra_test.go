...
func TestWorkflowAgent_MapToADKEvent_ConfirmationUsesNativeADKHITL(t *testing.T) {
	wa := &WisDevWorkflowAgent{}
	mctx := &mockInvocationContext{}
	mctx.On("InvocationID").Return("inv-confirm")
	mctx.On("Session").Return(mockSession{})

	got := wa.mapToADKEvent(mctx, PlanExecutionEvent{
		Type:    EventConfirmationNeed,
		StepID:  "step-1",
		Message: "review before execution",
		Payload: map[string]any{
			"approvalId":    "approval-1",
			"approvalToken": "token-1",
			"action":        "research.verifyClaimsBatch",
		},
	})
	...
	assert.Equal(t, toolconfirmation.FunctionCallName, got.Content.Parts[0].FunctionCall.Name)
	assert.Equal(t, []string{"token-1"}, got.LongRunningToolIDs)

	confirmations := got.Actions.RequestedToolConfirmations
	require.Contains(t, confirmations, "approval-1")
	assert.Contains(t, confirmations["approval-1"].Hint, "review before execution")
	assert.False(t, confirmations["approval-1"].Confirmed)
	assert.True(t, got.Actions.SkipSummarization)

	original, err := toolconfirmation.OriginalCallFrom(got.Content.Parts[0].FunctionCall)
	require.NoError(t, err)
	assert.Equal(t, "approval-1", original.ID)
	assert.Equal(t, "research.verifyClaimsBatch", original.Name)
	assert.Equal(t, "step-1", got.Actions.StateDelta["stepId"])
	assert.Equal(t, "research.verifyClaimsBatch", got.Actions.StateDelta["action"])
}
