...
function Invoke-GoTests {
    ...
    try {
        ...
        go test ./internal/api ./internal/search ./internal/wisdev ./internal/rag ./internal/evidence ./internal/evidence/citations ./internal/telemetry ./internal/stackconfig ./internal/cli ./cmd/server ./cmd/wisdev ./pkg/wisdev -count=1 -parallel=1
    } finally {
        Pop-Location
    }
}
...
function Invoke-StaticReleaseChecks {
    ...
    foreach ($pattern in $blockedPatterns) {
        $hits = Select-String -Path ($files.FullName) -Pattern $pattern -SimpleMatch -ErrorAction SilentlyContinue |
            Where-Object {
                $relative = Get-RepoRelativePath -Base $RepoRoot -Full $_.Path
                -not ($allowedPathFragments | Where-Object { $relative -eq $_ })
            }
        if ($hits) {
            $formatted = ($hits | ForEach-Object {
                "$(Get-RepoRelativePath -Base $RepoRoot -Full $_.Path):$($_.LineNumber): $($_.Line.Trim())"
    ...
    foreach ($dockerfile in $dockerfiles) {
        ...
        if (-not (Test-Path -LiteralPath $dockerignore)) {
            $relativeContext = Get-RepoRelativePath -Base $RepoRoot -Full $contextDir
