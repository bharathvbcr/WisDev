...
Each endpoint:
  ...
  - Returns a ``StepArtifactEnvelope`` serialized as a flat artifact map via
    ``flatten_to_artifact_map``, so Go's ``normalizeStepArtifacts`` continues to work
...
async def dispatch_action(request: Request) -> dict[str, Any]:
    ...
    action not handled natively. The response is always a flat artifact map that
...
async def resolve_canonical_citations(req: ResolveCitationsRequest) -> dict[str, Any]:
    """Resolve citations with emitter-backed artifact flattening for Go ingress."""
...
async def verify_citations(req: VerifyCitationsRequest) -> dict[str, Any]:
    """Verify citations with emitter-backed artifact flattening for Go ingress."""
    raw_result: dict[str, Any] = {
