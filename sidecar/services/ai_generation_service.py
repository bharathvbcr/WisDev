...
from artifacts.emitters import emit_for_action, flatten_to_artifact_map
from services.model_config import get_model_location, resolve_generation_model_id
...
class AiGenerationService:
    ...
    def __init__(
        ...
        self.vertex_proxy_url = (
            os.environ.get("VERTEX_PROXY_URL")
            or ""
        )
    ...
    def _resolve_model_id_for_class(model_class: str) -> str:
        ...
        model_id = resolve_generation_model_id(normalized)
        if model_id:
            return model_id

        raise RuntimeError(
            "Missing model configuration: set scholar_models.json "
            "(or AI_MODEL_{TIER}_ID / AI_MODEL_DEFAULT_ID)"
        )
    ...
    async def _generate_native_structured(
        ...
        scholar_models.json, with AI_MODEL_* env overrides still supported.
        ...
        location = (
            ...
            or get_model_location("generative", default="global")
            or "global"
        )
        ...
        # API accepts light|standard|heavy class names ("balanced" maps to "standard").
    ...
    async def generate_with_thinking(
        ...
        model_name = (
            ...
            or resolve_generation_model_id("heavy")
        )
        if not model_name:
    ...
    def _ensure_wisdev_shape(action: str, result: dict[str, Any]) -> dict[str, Any]:
        """
        Enforce stable artifact key shapes for WisDev action outputs.
    ...
    def emit_wisdev_action_output(self, action: str, raw_result: dict[str, Any]) -> dict[str, Any]:
        """
        Convert raw action output into the stable artifact map.
        ...
        flattened = flatten_to_artifact_map(envelope)
    ...
    async def generate_wisdev_action_output(
        ...
        """
        Generate typed JSON and return emitter-backed artifact map output.
        """
