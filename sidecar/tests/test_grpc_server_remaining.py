...
_EMBEDDING_MODEL_ENV_KEYS = (
    "EMBEDDING_MODEL_PRIMARY_ID",
    "EMBEDDING_MODEL_STANDARD_ID",
    "EMBEDDING_MODEL_FALLBACK_ID",
)


def _use_repo_embedding_config(monkeypatch: pytest.MonkeyPatch) -> None:
...
async def test_structured_output_grpc_boundary_exercises_runtime_compatibility_filter():
    ...
    assert structured_call["model"] == "gemini-3.1-flash-lite"
...
@pytest.mark.asyncio
async def test_embed_grpc_boundary_exercises_runtime_compatibility_filter(monkeypatch):
    _use_repo_embedding_config(monkeypatch)
    ...
    expected_model = model_config.get_embedding_model("standard")
