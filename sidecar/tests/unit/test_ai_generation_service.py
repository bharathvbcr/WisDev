...
class TestResolveModelIdForClass:
    ...
    def test_falls_back_to_central_config_when_no_env(self, monkeypatch):
        monkeypatch.delenv("AI_MODEL_LIGHT_ID", raising=False)
        monkeypatch.delenv("AI_MODEL_DEFAULT_ID", raising=False)
        resolved = AiGenerationService._resolve_model_id_for_class("light")
        assert resolved

    def test_raises_when_no_config_resolves(self):
        with patch(
            "services.ai_generation_service.resolve_generation_model_id",
            return_value="",
        ):
