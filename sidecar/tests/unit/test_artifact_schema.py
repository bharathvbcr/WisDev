...
- flatten_to_artifact_map produces the exact keys Go's normalizeStepArtifacts expects.
...
class TestCanonicalCitation:
    ...
    def test_model_dump_wire_keys(self):
        c = CanonicalCitation.model_validate(
            {
                "id": "x",
                "title": "T",
        ...
        wire = c.model_dump_wire()
        # All camelCase keys that Go's normalizeStepArtifacts reads.
        assert wire["arxivId"] == "0000.00001"
        assert wire["canonicalId"] == "cid"
        assert wire["resolved"] is True
        assert wire["year"] == 2020
...
class TestReasoningSchemas:
    ...
    def test_verification_wire_keys(self):
        v = ReasoningVerification.model_validate(
            {
                "totalBranches": 5,
                "verifiedBranches": 3,
...
# ---------------------------------------------------------------------------
# StepArtifactEnvelope + flatten_to_artifact_map
# ---------------------------------------------------------------------------
...
class TestStepArtifactEnvelope:
    ...
    def test_flatten_artifact_passthrough(self):
    ...
    def test_flatten_typed_overrides_passthrough_on_conflict(self):
        """Typed bundle should win over artifact passthrough for the same key."""
        ...
        env = StepArtifactEnvelope.model_validate(
            {
                ...
                "artifacts": {"citations": []},  # stale stale entry
            }
        )
        leg = flatten_to_artifact_map(env)
        # Typed bundle overwrites the stale empty list.
