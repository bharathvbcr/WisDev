...
class TestModuleFunctions:
    def test_load_model_tier_no_file_returns_default(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: [tmp_path / "missing-scholar-models.json"],
        )
    ...
    def test_load_model_tier_reads_json_file(self, tmp_path, monkeypatch):
        ...
        model_file = tmp_path / "scholar_models.json"
        model_file.write_text(json.dumps(model_data))
        model_config.clear_model_config_cache()
        monkeypatch.setattr(model_config, "_candidate_config_paths", lambda: [model_file])
        result = _load_model_tier("heavy", "fallback")
        model_config.clear_model_config_cache()
        assert result == "custom-heavy"
