...
from services import genai_native_config
...
def test_build_generate_content_config_drops_optional_fields(monkeypatch):
    ...
    config = genai_native_config.build_generate_content_config(
...
def test_build_embed_content_config_drops_optional_fields(monkeypatch):
    ...
    dropped: list[tuple[str, list[str]]] = []
    config = genai_native_config.build_embed_content_config(
...
def test_build_optional_thinking_config_reports_config_error(monkeypatch):
    genai_types = types.ModuleType("google.genai.types")
    ...
    config = genai_native_config.build_optional_thinking_config(
        {"thinking_level": "high"},
        operation="generate_structured",
        on_config_error=lambda operation, exc: captured.append((operation, str(exc))),
    )
